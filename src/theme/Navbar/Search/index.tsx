...
const MeilisearchSearchBar: React.FC<MeilisearchSearchBarProps> = ({
  ...
  const handleSearch = async (searchQuery: string) => {
        ...
        const rankedResults = (data.hits || []).map((result, index) => {
          const title = result.hierarchy?.lvl1 || result.hierarchy_lvl1 || result.hierarchy_radio_lvl2 || result.hierarchy_radio_lvl1 || result.hierarchy?.lvl2 || result.hierarchy_lvl2 || result.title || '';
