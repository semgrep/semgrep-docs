With **dataflow traces**, Semgrep Multimodal can provide you with a visualization of
...
When running Semgrep Multimodal from the command line, you can pass in the flag
`--dataflow-traces` to use this feature.

You can view dataflow traces in:

- Semgrep AppSec Platform by going to Semgrep Multimodal's [Findings page](https://semgrep.dev/orgs/-/findings?tab=open). For more details, see [Path of tainted data in Semgrep
  Code](/semgrep-code/semgrep-pro-engine-data-flow).
- The PR or MR comments created by Semgrep Multimodal running in your CI/CD system. To enable
  this feature, see:
