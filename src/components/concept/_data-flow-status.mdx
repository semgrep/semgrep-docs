In principle, the data-flow analyses engine (which provides taint tracking, constant propagation, and symbolic propagation) can run on any language [supported by Semgrep](/docs/supported-languages.md). However, the level of support is lower than for the regular Semgrep matching engine.
