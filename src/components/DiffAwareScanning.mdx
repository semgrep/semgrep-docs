To enable **diff-aware scanning**, set a `SEMGREP_BASELINE_REF` environment variable. Its value should be a **commit hash** or **branch name** (the baseline). For example, given a repository with 10 commits, setting `SEMGREP_BASELINE_REF` to the 8th commit will return results introduced by changes in the 9th and 10th commit.
