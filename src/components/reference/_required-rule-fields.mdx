...
| `severity`    | `string` | One of the following values: `INFO`, `WARNING`, or `ERROR`. The value of the `severity` key is assigned to a rule by Semgrep, Inc based on the values of a two other keys: `likelihood`  and `impact`. Note: Semgrep Supply Chain differs, as its rules use CVE assignments for severity calculation. |
