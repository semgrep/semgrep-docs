...
| `severity`    | `string` | One of the following values: `INFO` (low severity), `WARNING` (medium severity), or `ERROR` (high severity). The `severity` key specifies how critical are the issues that a rule potentially detects. Note: Semgrep Supply Chain differs, as its rules use CVE assignments for severity. |
