...
| **Ignored** | Findings that are ignored are present in the code, but have been labeled as unimportant. Ignore findings that are false positives or deprioritized issues. Mark findings as ignored through Semgrep App (see [Managing finding status](/semgrep-app/findings/#managing-finding-status-bulk-triage)) or by adding a code comment (see [nosemgrep comments](/ignoring-files-folders-code/#reference-summary)). |
...
Findings can also be **removed**. A removed finding does not count towards the fix rate or the number of findings. The removed finding also does not appear in the Semgrep App. A finding is removed if it is not found in the most recent scan of the branch where it was detected due to any of the following conditions:
