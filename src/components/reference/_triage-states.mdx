...
| **Ignored** | Findings that are ignored are present in the code, but have been labeled as unimportant. Ignore findings that are false positives or deprioritized issues. Mark findings as ignored through Semgrep Cloud Platform (see [Managing finding status](/semgrep-app/findings/#managing-finding-status-bulk-triage)) or by adding a code comment (see [nosemgrep comments](/ignoring-files-folders-code/#reference-summary)). |
