...
<Steps>
  <Step>
  Navigate to the repository that hosts the rule. Usually, the [<Icon icon="external-link" iconType="solid"/> Semgrep community rules repository](https://github.com/semgrep/semgrep-rules) hosts the rule, but Semgrep also imports rules from other repositories. From the rule's entry in the Registry, check the author of the rule to confirm the source. 
  </Step>
  <Step>
...
You can't use the `--config` option with `semgrep ci` once you are logged in. To use your custom rules, add them to your [<Icon icon="external-link" iconType="solid"/> Policies page](https://semgrep.dev/orgs/-/policies).
</Tip>
...
Dataflow and interfile analyses in particular ensure that Semgrep Supply Chain provides a high true positive rate while reducing false positives. Read the [<Icon icon="external-link" iconType="solid"/> Doyensec Software Composition Analysis Benchmark](https://www.doyensec.com/resources/Doyensec_Software_Composition_Analysis_Benchmark.pdf) to learn more.
</Tip>
