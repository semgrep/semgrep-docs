...
At either the organization or repository level, go to **Settings** > **Secrets and variables**
   <Frame>
   ![Secrets and variables section in Settings](/images/kb/semgrep-code/InvalidHeaderValue/github-secrets.png)
   </Frame>
</Step>
<Step>
Update the value of the `SEMGREP_APP_TOKEN` to ensure it does not have an extraneous newline (`\n`) and is not malformed
   <Frame>
   ![Updating GitHub Secret](/images/kb/semgrep-code/InvalidHeaderValue/github-update-value.png)
   </Frame>
</Step>
</Steps>
...
Update the `SEMGREP_APP_TOKEN` value to ensure it does not have an extraneous newline (`\n`) and is not malformed
   <Frame>
   ![Updating GitLab Variable](/images/kb/semgrep-code/InvalidHeaderValue/gitlab-update-value.png)
   </Frame>
</Step>
</Steps>
