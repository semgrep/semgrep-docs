...
<Tooltip tip="Abstraction that matches unknown values. Metavariables begin with $ and can include uppercase letters, digits, and underscores. See full definition.">Metavariable</Tooltip> matching operates differently for logical AND (`patterns`) and logical OR (`pattern-either`) parent operators. Behavior is consistent across all child operators: `pattern`, `pattern-not`, `pattern-regex`, `pattern-inside`, `pattern-not-inside`.

### Metavariables in logical ANDs

<Tooltip tip="Abstraction that matches unknown values. Metavariables begin with $ and can include uppercase letters, digits, and underscores. See full definition.">Metavariable</Tooltip> values must be identical across sub-patterns when performing logical AND operations with the `patterns` operator.
...
<Tooltip tip="Abstraction that matches unknown values. Metavariables begin with $ and can include uppercase letters, digits, and underscores. See full definition.">Metavariable</Tooltip> matching does not affect the matching of logical OR operations with the `pattern-either` operator.
...
<Tooltip tip="Abstraction that matches unknown values. Metavariables begin with $ and can include uppercase letters, digits, and underscores. See full definition.">Metavariable</Tooltip> matching still affects subsequent logical ORs if the parent is a logical AND.
