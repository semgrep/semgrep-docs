...
- Added a Jira API endpoint to create Jira tickets, either by passing a list of `issue_ids` or filter query parameters to select findings. Refer to the [<Icon icon="external-link" iconType="solid"/> Jira API documentation](https://semgrep.dev/api/v1/#tag/TicketingService/operation/semgrep_app.core_exp.notifications.ticketing.handlers.openapi_create_tickets).
...
- Added an API endpoint you can use to triage findings in bulk, either by passing a list of `issue_ids` or filter query parameters to select findings. Refer to [<Icon icon="external-link" iconType="solid"/> Bulk triage API documentation](https://semgrep.dev/api/v1/#tag/TriageService).
