...
SBOM generation can be performed through Semgrep AppSec Platform or the [<Icon icon="external-link" iconType="solid"/> Semgrep API](https://semgrep.dev/api/v1/#tag/SupplyChainService/operation/SupplyChainService_CreateSbomExport).
...
Refer to the [<Icon icon="external-link" iconType="solid"/> Semgrep API > SBOM documentation](https://semgrep.dev/api/v1/#tag/SupplyChainService/operation/SupplyChainService_CreateSbomExport). 
...
| References | Links to the specific CVE. References can come from NIST, Electron release notes, and GitHub Security <Tooltip tip="Announcement of a vulnerability, typically with an associated CVE. Semgrep Supply Chain rules can find advisories, and they appear in the Advisories view. See full definition and Advisories.">Advisory</Tooltip>. |
| <Tooltip tip="In taint analysis, code that introduces tainted data, typically user input. See full definition.">Source</Tooltip> | The primary source of this vulnerability's advisory. |
