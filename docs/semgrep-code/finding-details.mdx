...
- Suggested fix suggestions on how to fix findings from Semgrep Multimodal, when enabled
...
- Provide Suggested fix suggestions on how to fix findings and tag the finding with a component tag, such as `auth` or `payments`.
