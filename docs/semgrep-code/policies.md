...
* Failing a CI job, resulting in blocking a PR or MR.
...
| Block     | Rules in **Block mode** prevent a PR or MR from merging if Semgrep Code detects a finding from these rules.<br />These rules display findings in:<ul><li>Developers' PRs or MRs</li><li>Semgrep Cloud Platform</li><li>User-defined notifications</li></ul>These are typically high-confidence, high-severity rules. |
...
To add custom rules from the Semgrep Editor, see [<i class="fa-regular fa-file-lines"></i> Setting code standards with the Policies page](/semgrep-code/editor/#setting-code-standards-with-the-policies-page).
...
| Ruleset  | Rules are also organized in rulesets. Rulesets are groups of rules related through a programming language, OWASP category, or framework.       | <ul><li><a href="https://semgrep.dev/p/comment"><i class="fas fa-external-link fa-xs"></i> Comment ruleset</a></li><li><a href="https://semgrep.dev/p/react"><i class="fas fa-external-link fa-xs"></i> React ruleset</a></li></ul>       |
