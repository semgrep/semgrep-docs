...
Semgrep generates a finding whenever it scans a repository, and one of its rules matches a piece of code in the repository. Since Semgrep scans a repository multiple times, it needs a way to track the same finding in a file over time. Semgrep does this using two types of fingerprints: `match_based_id` and `syntactic_id`.
...
_**Figure**. Semgrep AppSec Platform groups together the same finding that has been identified as present on multiple branches._
...
Semgrep's correlation of findings across branches based on their unique fingerprint allows for automatic consolidation of findings and makes it simpler for you to triage findings.
