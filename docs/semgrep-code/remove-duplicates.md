...
Semgrep generates a finding whenever it scans a repository, and one of its rules matches a piece of code. Since Semgrep scans a repository multiple times, it needs a way to track the same finding in a file over time. Semgrep does this using two types of fingerprints: `match_based_id` and `syntactic_id`.
...
This information is combined and then hashed before being stored for privacy.
