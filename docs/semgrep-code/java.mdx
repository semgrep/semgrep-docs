...
The code examples shown here are best viewed in **a separate Semgrep Playground tab** so that you can see the <Icon icon="star" color="#b968ff"/> purple star outline. This star marks the lines that contain false positives and are correctly identified and removed by Semgrep.
</Tip>
...
**Figure**. `detect-pattern-in-subclass`. <a href="https://semgrep.dev/playground/s/nJjjG" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> Open in  interactive Playground</a>.
