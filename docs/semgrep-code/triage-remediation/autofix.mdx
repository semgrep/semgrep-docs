...
* If Semgrep connects to your SCM through the [Semgrep Network Broker](/semgrep-ci/network-broker), use Network Broker `v0.45.2` or later and set `allowCodeAccess` to `true`. See [Use Semgrep Network Broker with Autofix](/semgrep-ci/network-broker#use-semgrep-network-broker-with-autofix).
