...
* Accept Amazon Bedrock, including Anthropic Claude models.
...
* If Semgrep connects to your SCM through the [Semgrep Network Broker](/semgrep-ci/network-broker), use Network Broker 0.45.2 or later and set `allowCodeAccess` to `true` for that SCM (it defaults to `false`). Older brokers reject Autofix requests with a 403 allowlist error. See [Use Semgrep Network Broker with Autofix](/semgrep-ci/network-broker#use-semgrep-network-broker-with-autofix).
