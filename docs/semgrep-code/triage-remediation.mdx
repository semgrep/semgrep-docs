...
| **Ignored** | Findings marked as ignored are present in the code but have been labeled as such due to them being deemed safe to ignore, a false positive, or an acceptable risk. You can change the finding's status to [ignored through Semgrep AppSec Platform](/semgrep-code/triage-remediation) or by adding a [nosemgrep code comment](/ignoring-files-folders-code#ignore-code-through-nosemgrep). In Semgrep AppSec Platform, you can filter for ignored findings using the reason provided: **False positive**, **Acceptable risk**, **No time**, **No triage reason**, **Duplicate**, and **Ignored via nosemgrep**. |
...
Findings with a status of **provisionally ignored** block pull requests and merge requests if the matching rule is included in a remediation policy where the action taken is **Block merge and comment on PR/MR** when the policy's conditions are met.
