...
Findings with a status of **provisionally ignored** block pull requests and merge requests if the matching rule is included in a blocking policy. This ensures that findings from rules that have been promoted to blocking status are always reviewed, to improve security posture.
