...
Findings with a status of **provisionally ignored** block pull requests and merge requests if the matching rule is included in a blocking policy. This helps improve your organization's security posture by ensuring that any findings from blocking rules are reviewed before code is merged.
