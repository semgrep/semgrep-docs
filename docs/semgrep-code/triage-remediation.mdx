...
<Note>
  **AI-powered detection findings and Provisionally ignored**
  ...
  Only rule-based Semgrep Code findings are moved to **Provisionally ignored**. [AI-powered detection](/semgrep-code/ai-powered-detection-concepts) findings are **not** automatically moved to **Provisionally ignored**, even if Multimodal autotriages them as false positives. AI-powered detection findings that you want to dismiss must be triaged manually to **Ignored** with reason **False positive**.
