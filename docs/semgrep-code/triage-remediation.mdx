...
<Note>
  **AI-powered detection findings and Provisionally ignored**
  ...
  To list AI-powered detection findings that Multimodal flagged as likely false positives, go to [**Code**](https://semgrep.dev/orgs/-/findings?tab=open), apply the **Detection method** (or **Scan type**) filter to **AI-powered**, and then apply the **Multimodal autotriage** filter set to **False positive**.
</Note>
