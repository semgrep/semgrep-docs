...
### Change rule behavior in Semgrep AppSec Platform

You can use [remediation policies](/semgrep-appsec-platform/unified-policies/get-started#manage-remediation-policies) to change the action taken by Semgrep whenever it detects a finding that matches a rule and to disable rules and rulesets you no longer want to use.
