...
**Semgrep Editor** allows you to write rules, verify their performance through tests, and add them to your organization's [detection policies](/semgrep-appsec-platform/unified-policies/overview) to enforce code standards and increase code security.
...
Structure mode is a UI-based rule writing editor that guides you through the process of writing a rule.
...
To rename a rule, enter the new name in the YAML editor's `id` field. Then, save the rule by entering <kbd>Ctrl</kbd>+<kbd>S</kbd> (<kbd>⌘</kbd>+<kbd>S</kbd> on Mac) or clicking the **Save** button.
...
Do one of the following:
   * Open the rule, click the <Icon icon="ellipsis-vertical" iconType="solid"/> three vertical dots, then click <Icon icon="trash-can" iconType="regular"/> **Delete**.
   * Right-click the rule file in the navigation, then click <Icon icon="trash-can" iconType="regular"/> **Delete**.
</Step>
<Step>
...
The **[Detection Policies](https://semgrep.dev/orgs/-/detection)** page displays information about the rules that Semgrep uses to scan your project's code. To add a rule from the editor to a policy:
...
<Step>
Go to **Rules & Policies > Editor**, and open the rule you want to add to a policy.
</Step>
<Step>
Click **Add to detection <Tooltip tip="A policy defines the set of rules that Semgrep runs and the workflow actions it undertakes when a rule from the policy generates a finding. The workflow action can include notifying Slack channels or posting a comment in the pull request or merge request that generated the finding. Not to be confused with policy-as-code." cta="See full definition." href="/semgrep-code/glossary#policy">policy</Tooltip> > Add to Code detection policy**.
</Step>
<Step>
Once the rule is added to a detection policy, Semgrep uses it for subsequent full and diff-aware scans. To learn how to modify any of Semgrep's scanning behavior, see [Manage detection policies](/semgrep-appsec-platform/unified-policies/get-started).
</Step>
</Steps>
...
When you fork a rule, the copy is independent from the original. To run your new rule in your scans, [add it to a policy](#add-a-rule-to-a-policy). If you want your copy to replace the rule you forked, add it to a policy, then [disable the original in your detection policy](/semgrep-appsec-platform/unified-policies/get-started#manage-detection-policies).
