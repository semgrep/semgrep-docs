...
<Info>
**PREREQUISITE**
Not all Semgrep rules or rulesets make use of dataflow traces or taint tracking. Ensure that your [remediation policy](/semgrep-appsec-platform/unified-policies/overview) includes rules that use dataflow traces. Semgrep's default ruleset includes rules that use dataflow traces. If you're using a custom ruleset, add rules from the [Semgrep Registry](https://semgrep.dev/explore) that use dataflow traces to see results.
</Info>
