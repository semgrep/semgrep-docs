...
* Stand-alone CI jobs cannot block a pull or merge request based on a finding. There is no distinction or tiering between rules. Alternatively, Semgrep Cloud Platform's [Rule Board](/semgrep-app/rule-board) can tier rules and undertake actions, such as blocking PRs or MRs on a finding generated by a reliable rule.
...
2. Click **[Projects](https://semgrep.dev/orgs/-/projects)** > **Scan New Project** and follow the steps in [Integrating Semgrep in CI providers with Semgrep Cloud Platform](/semgrep-app/getting-started-with-semgrep-app).
3. Optional: If you have previously set a custom `SEMGREP_TIMEOUT` environment variable, commit it to the CI configuration file created by Semgrep Cloud Platform. Do not copy `SEMGREP_RULES`.
