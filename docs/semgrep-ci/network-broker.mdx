...
- [Autofix](/semgrep-code/triage-remediation/autofix) pull requests and merge requests
...
For more than one SCM instance, see [Allowlist multiple source code managers with one configuration file](#allowlist-multiple-source-code-managers-with-one-configuration-file).

The SCM-specific keys shown in the other tabs, such as `gitlab`, are still supported.

</Tab>
<Tab title="v0.25.0 to v0.46.0">
...
The `scms` list in v0.47.0 and later is an exception. The broker matches entries that have the same `type` and `baseUrl` and combines them into one entry. It keeps a key that only one file sets, such as `token`. If both files set the same key, it uses the value from the later file. Entries with a different `type` or `baseUrl` are both kept. A later file does not replace the list.
...
### Provide the private key through the environment

In Semgrep Network Broker v0.46.0 and later, you can pass the private key with an environment variable instead of `privateKey` in `config.yaml`. Use this when the key lives in a secret manager.

Set one of the following variables when you start the broker:

- `SEMGREP_NETWORK_BROKER_PRIVATE_KEY`: the private key, as printed by `genkey`.
- `SEMGREP_NETWORK_BROKER_PRIVATE_KEY_PATH`: the path to a file containing the private key, such as a mounted Docker or Kubernetes secret. A trailing newline in the file is ignored.

<Tabs>
<Tab title="Docker with an environment variable">

Pass the key when you [start the broker](#start-the-broker):

```bash
sudo docker run -d -it --rm -v $(pwd):/emt \
  -e SEMGREP_NETWORK_BROKER_PRIVATE_KEY="YOUR_PRIVATE_KEY" \
  ghcr.io/semgrep/semgrep-network-broker:VERSION_NUMBER -c /emt/config.yaml
```

<Warning title="KEY SHARING">
This command contains your private key. Do **not** share your private key with anyone, including Semgrep.
</Warning>

</Tab>
<Tab title="Docker with a mounted file">

Save the private key to a file named `privateKey` in the same directory as `config.yaml`. This command mounts that directory at `/emt`. Pass the path inside the container:

```bash
sudo docker run -d -it --rm -v $(pwd):/emt \
  -e SEMGREP_NETWORK_BROKER_PRIVATE_KEY_PATH=/emt/privateKey \
  ghcr.io/semgrep/semgrep-network-broker:VERSION_NUMBER -c /emt/config.yaml
```
...
```bash
kubectl create secret generic broker-private-key --from-literal=privateKey=YOUR_PRIVATE_KEY
```
...
Then reference the secret from the broker's Kubernetes Deployment, such as the one in the [sample Kubernetes manifest](https://github.com/semgrep/semgrep-network-broker/blob/develop/kubernetes.yaml). You can either inject the secret as an environment variable or mount it as a file.

**Option 1: Inject the secret as an environment variable.** Put `env` in the container spec:

```yaml
env:
  - name: SEMGREP_NETWORK_BROKER_PRIVATE_KEY
    valueFrom:
      secretKeyRef:
...
**Option 2: Mount the secret as a file** and point the broker at it. `env` and `volumeMounts` belong in the container spec, and `volumes` in the pod spec:

```yaml
env:
  - name: SEMGREP_NETWORK_BROKER_PRIVATE_KEY_PATH
    value: /secret/privateKey
volumeMounts:
  - mountPath: /secret
    name: broker-private-key-volume
    readOnly: true
volumes:
  - name: broker-private-key-volume
    secret:
...
- If you set either variable, the broker uses that key, ignores `privateKey` in the config file, and logs a warning that it overrode the config file value.
- If you set both variables, the broker uses `SEMGREP_NETWORK_BROKER_PRIVATE_KEY`.
- If `SEMGREP_NETWORK_BROKER_PRIVATE_KEY_PATH` points to a missing or empty file, the broker exits with an error.
- The private key must be exactly 44 characters, which decodes to 32 bytes. Otherwise, the broker exits with an error that names the source of the key.
...
## Use Semgrep Network Broker with Autofix

To use [Autofix](/semgrep-code/triage-remediation/autofix) through the Network Broker:

1. Upgrade to Network Broker 0.45.2 or later. Older versions return a 403 allowlist error.
2. Set `allowCodeAccess` to `true` for that SCM. This setting defaults to `false`. See [Use Semgrep Network Broker with Managed Scans](#use-semgrep-network-broker-with-managed-scans) for examples.

Grant Semgrep [read and write access](/semgrep-appsec-platform/scm-code-access) on the SCM connection.

If you don't want Semgrep to write to your repositories, leave `allowCodeAccess` unset or set it to `false`.
...
You can allow access to multiple source code managers (SCM) within one configuration file.

<Tabs>
<Tab title="v0.47.0 and later">

Add one entry to `scms` for each SCM instance. You can add several instances of the same type, such as two GitHub Enterprise Server instances:
...
Each entry requires `type` and `baseUrl`, and accepts `token` and `allowCodeAccess`. See the [broker README](https://github.com/semgrep/semgrep-network-broker?tab=readme-ov-file) for the endpoints each SCM type allows.
...
<Tab title="v0.46.0 and earlier">

One entry for a given SCM [uses the SCM-specific key provided in the configuration file](/semgrep-ci/network-broker#update-the-config-with-your-scm-information), as shown in the following example for a GitHub connection:
