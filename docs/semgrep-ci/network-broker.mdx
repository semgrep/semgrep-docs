...
The `scms` list in v0.47.0 and later is an exception: it is merged rather than replaced. Entries that share a `type` and `baseUrl` merge the way maps do, so a later file can set `token` or `allowCodeAccess` on an entry an earlier file defined. All other entries are kept.
...
You can allow access to multiple source code managers (SCM) within a single configuration file.
