...
Semgrep Network Broker versions 0.46.0 and later can also read the private key from the environment. This is an alternative to setting `privateKey` in `config.yaml`, useful if you prefer to keep the key in your platform's secret store. Set one of the following variables when you start the broker:
...
Save the private key to a file named `privateKey` in the same directory as `config.yaml`. The directory is already mounted at `/emt` when you [start the broker](#start-the-broker), so pass the path inside the container:
...
Create a secret holding the private key:
...
Then reference the secret from the broker Deployment. Either set the variable directly in the container's `env`:
...
- If the environment supplies a key, the broker uses it even when a config file also sets `privateKey`, and logs a warning that the config file value was overridden.
- If both variables are set, `SEMGREP_NETWORK_BROKER_PRIVATE_KEY` is used.
- If `SEMGREP_NETWORK_BROKER_PRIVATE_KEY_PATH` points to a missing or empty file, the broker exits with an error.
- The private key must be exactly 44 characters, which decodes to 32 bytes. Otherwise the broker exits with an error that names where the key came from.
