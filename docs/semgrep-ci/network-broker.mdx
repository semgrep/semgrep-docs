...
Create a Kubernetes Secret to store the private key. Add that secret to the broker Deployment in one of these ways.
...
Then reference the secret from the broker Deployment.

2. Mount the secret as a file. Put `env` and `volumeMounts` in the container spec, and `volumes` in the pod spec:
...
Or mount the secret as a file and point the broker at it. `env` and `volumeMounts` belong in the container spec, and `volumes` in the pod spec:
...
See the [sample Kubernetes manifest](https://github.com/semgrep/semgrep-network-broker/blob/develop/kubernetes.yaml) for a complete Deployment.
