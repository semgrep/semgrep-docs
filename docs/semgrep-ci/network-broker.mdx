...
[Autofix](/semgrep-code/triage-remediation/autofix) is available when Semgrep connects to your SCM through the Network Broker, subject to two requirements:

1. Upgrade to Network Broker 0.45.2 or later. Older brokers reject Autofix requests with a 403 allowlist error.
2. Set `allowCodeAccess` to `true` for that SCM in your broker configuration. This setting defaults to `false`. See [Use Semgrep Network Broker with Managed Scans](#use-semgrep-network-broker-with-managed-scans) for examples.

Also grant Semgrep [read and write access](/semgrep-appsec-platform/scm-code-access) on the SCM connection.
