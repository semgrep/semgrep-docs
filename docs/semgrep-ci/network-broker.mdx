...
[Autofix](/semgrep-code/triage-remediation/autofix) can create a branch, commit a fix, and open a pull request or merge request through the Network Broker.

To use Autofix when Semgrep connects to your SCM through the Network Broker:

1. Upgrade to Network Broker `v0.45.2` or later. Earlier versions do not allowlist all of the endpoints Autofix needs.
2. Set `allowCodeAccess` to `true` in your broker configuration. See [Use Semgrep Network Broker with Managed Scans](#use-semgrep-network-broker-with-managed-scans) for examples.
3. Grant Semgrep [read and write access](/semgrep-appsec-platform/scm-code-access) on the SCM connection.

Leave `allowCodeAccess` unset or set it to `false` if you do not want Semgrep to write to your repositories.
