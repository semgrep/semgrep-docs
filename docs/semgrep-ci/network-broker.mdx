...
`scms` is a list with one entry for each SCM instance. Each entry sets its own `type`, so you can add several instances of the same SCM. See [Allowlist multiple source code managers with one configuration file](#allowlist-multiple-source-code-managers-with-one-configuration-file).

The SCM-specific keys shown in the other tabs, such as `gitlab`, still work in v0.47.0 and later.
...
It is possible to allow access to multiple source code managers (SCM) within a single configuration file.
