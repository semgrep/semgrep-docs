...
`scms` is a list with one entry for each SCM instance. See [Allowlist multiple source code managers with one configuration file](#allowlist-multiple-source-code-managers-with-one-configuration-file).
...
If you use the `scms` key with an SCM list, available in v0.47.0 and later, set each entry's `type` to `azuredevops`, `bitbucket`, `github`, or `gitlab`, and set its `baseUrl` to that same base URL.
