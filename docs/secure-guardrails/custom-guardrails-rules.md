...
- Enabling [Code search (beta)](/semgrep-multimodal/editor#code-search-beta) is useful in verifying that your rule matches what you want it to match within your repositories.
...
| Verify that the rule matches as intended         |  <ul><li>See [Testing rules](/writing-rules/testing-rules).</li><li>Enable [Code search (beta)](/semgrep-multimodal/editor#code-search-beta) to test the rule on live repositories.</li></ul>      |
...
| Deploy the rule as a guardrail         |  For PR or MR comments: <ul><li>[Ensure that PR or MR comments have been set up correctly.](/category/pr-or-mr-comments)</li><li>Set the rule to [Comment or Block mode](/semgrep-multimodal/policies#block-a-pr-or-mr-through-rule-modes).</li></ul><p>For IDEs: Require developers to install the [Semgrep extension for their IDE](/extensions/overview).</p><p>For `pre-commit`: [Install and configure Semgrep for `pre-commit`](/extensions/pre-commit).</p> |
