...
| :--- | :--- | :--- | :--- |
| IDE | To notify developers of findings from a rule, add the rule to your Code remediation policy. | To notify developers of findings from a rule, add the rule to your remediation policy for multiple finding types. | To notify developers of findings from a rule, add the rule to your Supply Chain remediation policy. |
| PR or MR comments | To notify developers, create a remediation policy with the **Comment on PR/MR** action that triggers when Semgrep identifies a finding and the conditions of the policy are met. | To notify developers, create a remediation policy with the **Comment on PR/MR** action that triggers when Semgrep identifies a finding and the conditions of the policy are met. You can configure your policy's conditions to include only **high confidence, high severity rules**. | Developers receive comments about any **reachable vulnerability of high or critical severity due to the default Semgrep remediation policy.** |
| CLI through `pre-commit` | To notify developers of findings from a rule, create a remediation policy with the **Comment on PR/MR** action that triggers when Semgrep identifies a finding and the conditions of the policy are met. | To notify developers of findings from a rule, create a remediation policy with the **Comment on PR/MR** action that triggers when Semgrep identifies a finding and the conditions of the policy are met. | Developers are notified of **all** findings by default. |
