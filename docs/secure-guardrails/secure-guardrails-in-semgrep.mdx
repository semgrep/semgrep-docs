...
Semgrep Multimodal's **Suggested fix** includes suggestions on how to fix the insecure code. Suggested fix does **not** include inline code diffs.
...
<Frame caption="Suggested fix suggestions on how to fix a finding in a pull request comment.">
...
- Within developer-native interfaces, Semgrep Multimodal only appears in PR or MR comments. Suggested fix guidance does not appear in the IDE or `pre-commit`.
- You can adjust when Suggested fix guidance is shown to developers based on the level of confidence in the guidance.
- For AI-generated code changes, use [Autofix](/semgrep-code/triage-remediation/autofix) from Semgrep AppSec Platform.
</Note>
