...
Semgrep Guardian integrates natively with AI coding agents like Claude Code and Cursor to catch security issues before they ship. It bundles the Semgrep MCP server, Hooks, and Skills into a single install, and scans every file an agent generates using Semgrep Code, Supply Chain, and Secrets. When findings are detected, Guardian returns them to the agent, which decides whether to regenerate the code. Guardian does not block the write, so an agent may proceed despite a finding.
