...
The `guardian-default` ruleset draws on [`p/default`](https://semgrep.dev/p/default) and favors rules where a fix or a sanitizer is always available for the agent to apply. Rules without a reliable fix can lead an agent to rewrite code incorrectly, so they stay out of the ruleset.

Coverage is strongest for Python, Java, JavaScript, and TypeScript. Semgrep recently audited and expanded the ruleset, so if you have run Guardian before, expect more findings than in earlier versions.

If your agent does something that Guardian does not catch, report it in the `#mcp` [Slack community](https://go.semgrep.dev/slack) so that Semgrep can consider it for coverage.
