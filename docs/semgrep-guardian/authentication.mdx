...
If each developer cannot sign in individually, you can roll out Guardian with a read-only token instead. This is the one supported shared-credential path, and it requires Semgrep Enterprise. For more information, see [Roll out with a read-only token](/semgrep-guardian/enterprise-deployment#roll-out-with-a-read-only-token-semgrep-enterprise-only).
