...
{/* TODO: Confirm the boundaries between Agentic Workflows and AI-powered detection with Product. In particular, confirm whether AI-powered detection findings are still the correct public term. */}
...
When you start Agentic Workflows, Semgrep accesses the selected repository through its source code manager connection. The workflow runs its jobs and produces **Findings**. Semgrep processes and deduplicates those findings into reviewable **Issues**. You can monitor the run and review its issues on the **Workflows** page in Semgrep AppSec Platform.

{/* TODO: Verify every term against the UI and API schema. Add status definitions for runs and jobs, including queued, running, completed, failed, and canceled, if applicable. Document retry and cancellation semantics. Product name in docs is Agentic Workflows; some UI surfaces use Workflows (for example, left-hand nav). Confirm remaining UI strings (activation prompts use Agentic Workflows). */}
