...
**Workflow jobs** and **Workflow issues** lists, including groups and counts, show only jobs and issues for projects or repositories you can access. On deployments that use [Teams](/deployment/teams/overview), ticket creation follows the same scope as triage.

To review issues for a project:

1. In Semgrep AppSec Platform, go to **Projects**.
2. In the **Workflow issues** column, select the count for that project.

To review issues across your organization:
...
On **Workflows > Issues**, click the **<Icon icon="download" iconType="regular"/> icon** near **Group and sort** to export issues to a **CSV** file.

Exports respect the filters and sorting applied on the page, and are limited to 10,000 issues at a time. If more than 10,000 issues match your filters, narrow the filters until export is available.

You can also export issues through the experimental [Issues export API](/api-reference/v2/issuesservice/export-issues). See [API access](#api-access).
