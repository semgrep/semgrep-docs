...
When creating a new token, you are able to set a token's **scope**. Token scopes enable you to limit or grant permissions as necessary. Tokens are also autogenerated with appropriate scopes by Semgrep Cloud Platform based on the user's role or when onboarding a repository.
...
The following table displays typical use cases for token scopes.
| Token scope |Typical use case |
| ----------- | --------------- |
| Agent (CI)  | Autogenerated by Semgrep Cloud Platform when onboarding (adding) a repository to Semgrep Cloud Platform. |
| Web API     | Manually selected to access Semgrep's API or when additional permissions are required to enable PR or MR comments. | 
| Member      | Autogenerated by Semgrep CLI when a member is logging in through Semgrep CLI. |
