...
| ----------- | ------- | --- | --| -- |
| Agent (CI)  |   ✔️  Yes    |  ✔️ Yes  | ✔️  Yes | ✔️  Yes  |
| Web API     |  ❌ No    |  ❌ No | ✔️  Yes  | ✔️ Yes  |
| Member      |    ❌ No   |  ✔️ Yes  | ❌ No |❌ No |

:::note
- Member-scoped tokens are only available to organizations with RBAC enabled. If RBAC is not enabled, users are able to create and edit tokens with any scope.
- Within organizations with RBAC, only the `admin` role is able to edit, view and create tokens.
:::
...
| ----------- | ----------- |
| Agent (CI)  | Generated by Semgrep Cloud Platform when onboarding (adding) a repository to Semgrep Cloud Platform. This is the most permissive scope. |
