...
In this document, we discuss two attacks related to XML external entities: exponential entity expansion, and entity injection. In Java, applications are secure from exponential entity expansion by default, so no mitigations are necessary. To protect an XML parser in Java from XML External Entity Injection, on the other hand, many counter measures exist, not all of them effective. In this cheat sheet, we discuss what these measures are and test their availablity and effect on several Java classes that process XML documents.
...
When a document that contains an XML bomb is parsed, the parser expands each of the nested references. Consequently, this expansion becomes exponential leading to excessive use of resources and Denial-of-Service (DoS).
...
[`javax.xml.parsers.SAXParserFactory`](https://docs.oracle.com/javase/7/docs/api/javax/xml/parsers/SAXParserFactory.html) can process all 10 attack payloads. In its default configuration, it is vulnerable to 4 of the attack payloads.
...
[`javax.xml.parsers.SAXParser`](https://docs.oracle.com/javase/7/docs/api/javax/xml/parsers/SAXParser.html) can process all 10 payloads. In its default configuration, it is vulnerable to 4 attack payloads.
...
[`org.dom4j.io.SAXReader`](https://dom4j.github.io/javadoc/2.0.1/org/dom4j/io/SAXReader.html) can process all 10 payloads. In its default configuration, it is vulnerable to 4 of the attack payloads.
