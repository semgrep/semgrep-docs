...
The `exec` executes the specified string command in a separate process. This is dangerous if a command string is controlled by user input and could result in a command injection.
...
- Avoid running a shell as a command with arguments. For example `Runtime.getRuntime().exec("bash", "-c", userInput)` TODO specify
...
- Try to avoid non-literal values for the command string, such as `Runtime.getRuntime().exec("bash", "-c", input)` in the example above. Do not use `input` and similar functions.
