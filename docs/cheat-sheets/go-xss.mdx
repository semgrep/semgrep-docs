...
This is a cross-site scripting (XSS) prevention cheat sheet by r2c. It contains code patterns of potential XSS in an application. Instead of scrutinizing code
for exploitable vulnerabilities, the recommendations in this cheat sheet pave a safe road for developers that mitigates the possibility of XSS in your code. By  following these recommendations, you can be reasonably sure your code is free of XSS.
...
`tempalte.HTML` is a special type which instructs the template engine not to escape the content.
...
`tempalte.HTMLAttr` is a special type which instructs the template engine not to escape the content.
...
`tempalte.CSS` is a special type which instructs the template engine not to escape the content in CSS contexts.
...
`tempalte.JS` is a special type which instructs the template engine not to escape the content in JavaScript contexts, such as between `script` tags.
...
`tempalte.JSStr` is a special type which instructs the template engine not to escape the content when in JavaScript contexts and in a string.
...
`tempalte.Srcset` is a special type which instructs the template engine not to escape the content.
...
`tempalte.URL` is a special type which instructs the template engine not to escape the content.
