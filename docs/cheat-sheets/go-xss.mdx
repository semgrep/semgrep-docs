...
`template.JS` is a special type which instructs the template engine not to escape the content in JavaScript contexts, such as between `script` tags.
...
`template.JSStr` is a special type which instructs the template engine not to escape the content when in JavaScript contexts and in a string.
...
`template.URL` is a special type which instructs the template engine not to escape the content.
