...
Try to avoid non-literal values for the command string. If it is not possible, then do not let running arbitrary commands, use an allow list for inputs. Strip `!@#$;&*~"'{}][-+%^` characters from an input provided for the command string. 
...
Try to avoid non-literal values for the command string. If it is not possible, then do not let running arbitrary commands, use an allow list for inputs.
