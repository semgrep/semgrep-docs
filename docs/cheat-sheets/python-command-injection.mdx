...
- Avoid running shell as a command with arguments, e.g. `subprocess.run(["bash", "-c", user_input])`
...
- Don’t pass user-controlled input.
- Use `shlex.split` to correctly parse a command string into an array and `shlex.quote` to correctly sanitize input as a command-line parameter.
- Avoid running shell as a command with arguments, e.g. `os.spawnve(os.P_WAIT, "/bin/bash", ["-c", user_input], os.environ)`
...
- Don’t pass user-controlled input.
- Use `shlex.split` to correctly parse a command string into an array and `shlex.quote` to correctly sanitize input as a command-line parameter.
- Avoid running shell as a command with arguments, e.g. `os.execve("/bin/bash", ["/bin/bash", "-c", user_input], os.environ)`
- If it’s not possible to avoid, strip everything except alphanumeric characters from an input provided for the command string and arguments.
...
- Don’t pass user-controlled input.
- Use `shlex.split` to correctly parse a command string into an array and `shlex.quote` to correctly sanitize input as a command-line parameter.
- Avoid running shell as a command with arguments, e.g. `asyncio.subprocess.create_subprocess_exec("bash", ["bash", "-c", user_input])`
- If it’s not possible to avoid, strip everything except alphanumeric characters from an input provided for the command string and arguments.
