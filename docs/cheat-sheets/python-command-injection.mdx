...
- Avoid running shell as a command with arguments. For example: `subprocess.run(["bash", "-c", user_input])` TODO specify
...
- Don’t pass user-controlled input.
- Use `shlex.split` to correctly parse a command string into an array and `shlex.quote` to correctly sanitize input as a command-line parameter.
- Avoid running shell as a command with arguments. For example: `os.spawnve(os.P_WAIT, "/bin/bash", ["-c", user_input], os.environ)` TODO specify
...
- Don't pass user-controlled input.
- Use `shlex.split` to correctly parse a command string into an array and `shlex.quote` to correctly sanitize input as a command-line parameter.
- Avoid running shell as a command with arguments. For example: `os.execve("/bin/bash", ["/bin/bash", "-c", user_input], os.environ)` TODO specify
- If it's not possible to avoid, strip everything except alphanumeric characters from an input provided for the command string and arguments.
...
- Don't pass user-controlled input.
- Use `shlex.split` to correctly parse a command string into an array and `shlex.quote` to correctly sanitize input as a command-line parameter.
- Avoid running shell as a command with arguments. For example: `asyncio.subprocess.create_subprocess_exec("bash", ["bash", "-c", user_input])` TODO specify
- If it's not possible to avoid, strip everything except alphanumeric characters from an input provided for the command string and arguments.
