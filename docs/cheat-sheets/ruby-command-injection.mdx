...
- Avoid running a shell as a command with arguments, e.g. `Open3.pipeline(["bash", "-c", user_input])`
...
### 1.B. Using open() tu run subprocesses

Creates an input/output (I/O) object connected to a stream, file, or subprocess. If the first argument starts with a pipe character (`|`), it creates a subprocess. An opportunity for command injection vulnerability is created when the subprocess includes user input in a command argument to `open()` function.
...
- Do not let user input in `open` methods.
- Always try to use internal Ruby API (if it exists) instead of running an OS command. In other words, use internal language features instead of invoking commands that can be exploited.
- Don’t pass user-controlled input or use an allowlist for inputs.
- Avoid running a shell as a command with arguments. TODO add for example:
- Define a list of allowed arguments.
