...
The `spawn()` executes OS commands. This might potentially lead to command injection vulnerability when used with user input. The malicious actor can potentially run OS commands to exploit the system.
...
### 1.E. Using backticks (``) or %x[command]

Backticks ``` `` ``` or `%x[command]` methods execute a system command and return its output. Both methods accept string interpolation. As for other methods mentioned in this cheat sheet, when this method is used with user input, it can lead to command injection vulnerability.
...
# from: TODO remove https://stackoverflow.com/questions/6338908/ruby-difference-between-exec-system-and-x-or-backticks
%x[ ls -lah /tmp ]
%x{ ls -lah /tmp }
...
The `ls -l` in Ruby is equivivalent to the following:
...
- [PTY](https://ruby-doc.org/3.2.1/exts/pty/PTY.html) documentation
