...
The `html_safe()` decorator adds the `__html__` magic method to the supplied class. The added `__html__` magic method will return the exact string representation of the class (for example, `str(self)`). Because objects with the `__html__` method are not escaped, this could create a XSS vulnerability.
