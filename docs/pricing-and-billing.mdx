...
<dd>The Team tier includes Semgrep Code and Semgrep Supply Chain (SSC), which enables users to scan both first-party and third-party open source dependencies. It also includes Semgrep Cloud Platform, a web app enabling users to manage users, organizations, repositories, and scans. Team tier also includes <strong>cross-function (intrafile)</strong> and <strong>cross-file (interfile) analysis</strong> through <a href="/semgrep-code/semgrep-pro-engine-intro/">Semgrep Pro Engine</a>.</dd>
