...
The new syntax supports taint mode, and such rules no longer require `mode: taint` in the rule. Instead, everything must be nested under a top-level `taint` key.
