...
* <Tooltip tip="Abstraction that matches unknown values. Metavariables begin with $ and can include uppercase letters, digits, and underscores." cta="See full definition." href="/writing-rules/glossary#metavariable">Metavariable</Tooltip> support is limited to capturing a single “word”, which is a token of the form [A-Za-z0-9_]+. They can’t capture sequences of tokens such as hello, world (in this case, there are three tokens: `hello`, `,`, and `world`).
...
This pattern matches the input file but assigns the value `p` to `$PASSWORD` instead of the full value `p@$$w0rd`.
...
1. Use a named ellipsis by changing the pattern to the following:
 ...
 This still leads Semgrep to capture too much information. The value assigned to `$...PASSWORD` is now `p@$$w0rd` and<br />
