...
An anonymous metavariable does not produce any binding to the code it matched. This means it does not store the code that it matched. The pattern:
...
For search mode rules, metavariables with the same name are treated as the same metavariable across and within pattern operators **except for `pattern-either`**. This is called metavariable unification.
