...
Functions and classes can be referenced by their fully qualified name, for example,
...
:::tip
Half or partial statements can't be matches; both of the examples above must specify the contents of the condition’s body (e.for example`$BODY` or `...`), otherwise they are not valid patterns.
:::
...
those constructs, for example, Ruby).
...
Using [`options`](../rule-syntax/#options) it is possible to entirely disable AC-matching. It is also possible to treat boolean AND and OR operators (for example, `&&` in `||` in C-family languages) as commutative, which can be useful despite not being semantically accurate.
...
To search for specific strings, use the syntax `"=~/<regexp>/"`. Advanced regexp features are available, such as case-insensitive regexps with `'/i'` (for example, `"=~/foo/i"`). Matching occurs anywhere in the string unless the regexp `^` anchor character is used: `"=~/^foo.*/"` checks if a string begins with `foo`.
