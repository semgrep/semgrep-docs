...
This document describes Semgrep’s pattern syntax. You can also see pattern [examples by language](/docs/pattern-examples). In the command line, patterns are specified with the flag `--pattern` (or `-e`). Multiple
coordinating patterns may be specified in a configuration file. See
[rule syntax](/docs/rule-syntax) for more information.
...
or in a [`metavariable-regex`](/docs/rule-syntax/#metavariable-regex).
...
Experimentally, Semgrep also supports [intra-procedural flow-sensitive constant propagation](/docs/data-flow/constant-propagation).
...
Using [`options`](/docs/rule-syntax#options) it is possible to entirely disable AC-matching. It is also possible to treat boolean AND and OR operators (e.g., `&&` in `||` in C-family languages) as commutative, which can be useful despite not being semantically accurate.
...
[pattern-inside](/docs/rule-syntax#pattern-inside) to restrict the
...
:::warning
String matching has been deprecated. You should use [`metavariable-regex`](/docs/rule-syntax#metavariable-regex) instead.
:::
