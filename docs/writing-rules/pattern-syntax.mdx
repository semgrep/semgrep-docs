...
The ellipsis operator can also be used for the function name.
In some cases, you can match any function definitions:
regular functions, methods, but also anonymous functions (such as lambdas).
...
in Javascript the pattern `function ...($X) { ... }` matches
...
The `...` ellipsis operator matches everything in its sub-scope. See the following example:
...
To display matched metavariable in a rule message, add the same metavariable as you are searching for in your rule to the rule message. For example:

1. Find the metavariable used in the Semgrep rule. For example:
    ```yaml
...
:::info
If you're using Semgrep's advanced dataflow features, see documentation of experimental feature [Displaying propagated value of metavariable](/experiments/display-propagated-metavariable).
:::
