...
Anonymous metavariables are used to specify that there a metavariable exists in the pattern you want to capture.
...
For search mode rules, metavariables with the same name are treated as the same metavariable across and within pattern operators **except for `pattern-either`**.

For taint mode rules, metavariable unification is **not** enabled by default. To enforce unification, set `taint_unify_mvars: true` under the rule `options` key. When `taint_unify_mvars: true` is set, a metavariable defined in `pattern-sinks` and `pattern-sources` with the same name is treated as the same metavariable. See [Metavariables, rule message, and unification](/writing-rules/data-flow/taint-mode#metavariables-rule-message-and-unification) for more information.
