...
Rule-defined fixes are deterministic and user-defined. Related AI-powered features include [Semgrep Multimodal's Suggested fix](/semgrep-multimodal/overview#suggested-fix), which includes suggestions on how to fix findings, and [Autofix](/semgrep-code/triage-remediation/autofix), which can automatically generate a fix PR with a single click.
