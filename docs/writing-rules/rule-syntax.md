...
The full list of available options can be consulted in [Semgrep matching engine configuration](https://github.com/returntocorp/semgrep/blob/develop/interfaces/Config_semgrep.atd) module. Note that options not included in the table above are considered experimental, and they may change or be removed without notice.
