...
goal is to replace all the calls like `todo env x` by the construction
...
`Boilerplate.ml` (e.g. `languages/go/tree-sitter/semgrep-go/lib/Boilerplate.ml`). The `todo env x` calls are typically replaced by the
