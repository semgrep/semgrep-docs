...
1. In the `semgrep` repository, go to [`/src/parsing/Check_pattern.ml`](https://github.com/semgrep/semgrep/blob/develop/src/parsing/Check_pattern.ml), and add <PL>LANG</PL> to `lang_has_no_dollar_ids`. If the grammar has no dollar identifiers, add <PL>LANG</PL> above 'true'. Otherwise, add it above 'false'.
