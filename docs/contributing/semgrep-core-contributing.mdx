...
Note that since semgrep 0.82, you can pass the `--dump-command-for-core` (or the shorter `-d`) to `semgrep` to get the command the python wrapper will use to call semgrep-core (this is a hidden option, which is why you will not see it in `semgrep --help`). For example:
...
There is more information in [Add Support for a Language](#add-support-for-a-language) on tree-sitter which will be helpful.
