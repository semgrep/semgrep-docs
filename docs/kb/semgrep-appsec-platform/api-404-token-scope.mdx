...
Semgrep AppSec Platform supports two primary token scopes: **Agent (CI)** and **Web API**. When you create either scope, you can also assign a **read-only** role so the token can call only the read APIs. See [Token scopes](/deployment/tokens#token-scopes) for details of their permissions.
...
A **read-only** Web API token can authenticate to the Web API for read endpoints. It cannot perform write or mutate operations. If you need write access, create a new token with full Web API permission. You cannot change a read-only token's role after creation.
...
If the token is read-only, you cannot escalate it to full Web API access. Create a new token with the scopes you need instead.
