...
You can see results from the API that match those shown in Semgrep AppSec Platform by making a call to the [List code or supply chain findings](/api-reference/v1/findingsservice/list-code-supply-chain-or-ai-powered-detection-findings) endpoint, ensuring that you include the `dedup=true` parameter.
