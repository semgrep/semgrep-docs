...
- You have configured your GitHub app with permissions for all repositories that are scanned by Semgrep AppSec Platform. See [Enabling GitHub pull request comments](/semgrep-appsec-platform/github-pr-comments) for details, or review the following examples:
  ...
  <Frame caption="Figure. Grant permissions for access to all repositories.">
  ![Semgrep GitHub app permissions: all repositories](/images/kb/semgrep-appsec-platform/missing-pr-comments/gh-app-permissions-all.png)
  </Frame>

  <Frame caption="Figure. Grant permissions for access to select repositories. Ensure the repositories you have onboarded to Semgrep AppSec Platform are included here.">
  ![Semgrep GitHub app permissions - select repositories](/images/kb/semgrep-appsec-platform/missing-pr-comments/gh-app-permissions-select.png)
  </Frame>
...
This also means that baseline scans are not conducted if all findings in the current commit are in files added by the PR or MR, because those findings could not have been present before. This information is logged as:
...
To receive comments for a rule, you must configure the [remediation policy](/semgrep-appsec-platform/unified-policies/get-started#manage-remediation-policies) to include either the **Block merge and comment on PR/MR** action or the **Comment on PR/MR** action. Semgrep performs the configured action when it identifies a finding that meets the policy's conditions.
