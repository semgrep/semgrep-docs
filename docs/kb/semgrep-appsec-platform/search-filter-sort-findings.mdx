...
The **Multimodal autotriage** filter lists findings that Semgrep Multimodal evaluated as likely false positives. This includes both:
...
- **AI-powered detection findings**, which **remain Open** even when Multimodal flags them as false positives. AI-powered detection findings are not auto-triaged to **Provisionally ignored**, so you must use the **Multimodal autotriage** filter to find them.
...
<Steps>
  ...
  <Step>
    Optional: To narrow results to only AI-powered detection findings, also apply the **Detection method** (or **Scan type**) filter and select **AI-powered**.
  </Step>
</Steps>
