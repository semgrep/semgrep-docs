...
- Changing the role suffix in a group name, such as `[team:manager]` to `[team:readonly]`, doesn't update members' team roles.
...
If you have multiple role-suffixed groups for one team, update the team name consistently across all of them. Changing the role suffix, such as `[team:manager]` to `[team:readonly]`, doesn't update members' team roles.
