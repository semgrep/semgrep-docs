...
<Warning>
Any group you assign to the Semgrep application for role mapping also provisions its existing members if they're not already in your directory. Confirm the group's membership before assigning it.
...
Team groups that use the `[team:<role>]` naming convention also appear in the role mapping table. Don't assign a deployment role to these groups.
</Note>
...
Name the group with `[team]` or `[team:<role>]` before you assign it. A group without that suffix is treated as a deployment role group. Renaming a group that's already assigned doesn't trigger team creation. Create a new group with the naming convention, then assign it.

Assigning a group to the application provisions its members. Don't use a team group as a user's only assignment path into the application.
...
- You can assign a user to multiple role-suffixed groups for the same team. Semgrep assigns the highest-privilege team role among those groups (**Manager**, then **Member**, then **Read-only**).
- To lower a user's team role, remove them from the higher-privilege group.
- Don't rename the role suffix in a group name. Changing the suffix, such as `[team:manager]` to `[team:readonly]`, isn't supported. It doesn't update members' team roles and can cause unintended behavior.
...
If you have multiple role-suffixed groups for one team, update the team name consistently across all of them. Don't rename the role suffix. Changing the suffix, such as `[team:manager]` to `[team:readonly]`, isn't supported. It doesn't update members' team roles and can cause unintended behavior.
...
Semgrep doesn't delete the team until you remove all role-suffixed groups that back it. Deleting only one of those groups removes or demotes that group's members. Team deletion can take longer than a minute, especially for large teams.
...
<Step>
Select a role-suffixed group for the team you want to remove, then click **Remove assignment**.
</Step>
<Step>
Click **Yes** in the confirmation dialog.
</Step>
<Step>
Repeat for every remaining role-suffixed group for that team.
</Step>
</Steps>
...
- If a user belongs to multiple role-suffixed groups for the same team, Semgrep assigns the highest-privilege team role among those groups. The user remains on the team until you remove them from all of those groups.
- Renaming only one role-suffixed group renames the Semgrep team and leaves other role-type groups in Entra ID unmapped. Always update all role types together.
- Semgrep doesn't delete a team until all role-suffixed groups that back it are deleted. Deleting one group removes or demotes that group's members. Team deletion can take longer than a minute, especially for large teams.
- Don't rename the role suffix in a group name. Changing the suffix, such as `[team:manager]` to `[team:readonly]`, isn't supported. It doesn't update members' team roles and can cause unintended behavior.
