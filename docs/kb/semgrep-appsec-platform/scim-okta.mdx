...
- You can assign a user to multiple role-suffixed push groups for the same team. Semgrep assigns the highest-privilege team role among those groups (**Manager**, then **Member**, then **Read-only**).
...
If you have multiple role-suffixed push groups for one team, update the team name consistently across all of them. Don't rename the role suffix. Changing the suffix, such as `[team:manager]` to `[team:readonly]`, isn't supported. It doesn't update members' team roles and can cause unintended behavior.
...
Semgrep doesn't delete the team until you unlink and delete all role-suffixed push groups that back it. Deleting only one of those groups removes or demotes that group's members. Team deletion can take longer than a minute, especially for large teams.

<Steps>
<Step>
Under the **Push Groups** tab, find a role-suffixed group for the team you want to remove.
</Step>
<Step>
Click the **Active** drop-down, then select **Unlink Push Group**.
</Step>
<Step>
Select **Delete the group in the target app**.
</Step>
<Step>
Repeat for every remaining role-suffixed push group for that team.
</Step>
</Steps>
...
- If a user belongs to multiple role-suffixed push groups for the same team, Semgrep assigns the highest-privilege team role among those groups. The user remains on the team until you remove them from all of those groups.
- Renaming only one role-suffixed push group renames the Semgrep team and leaves other role-type groups in Okta unmapped. Always update all role types together.
- Semgrep doesn't delete a team until all role-suffixed push groups that back it are deleted. Deleting one group removes or demotes that group's members. Team deletion can take longer than a minute, especially for large teams.
- Don't rename the role suffix in a group name. Changing the suffix, such as `[team:manager]` to `[team:readonly]`, isn't supported. It doesn't update members' team roles and can cause unintended behavior.
