...
2. Re-run the scan with either the `-v`/`--verbose` or `--debug` (extremely verbose) flags. These options provide more information about what is failing. Keep in mind, to get the desired output, you will also likely have to set `--max-log-list-entries` to `0` (or any negative value) as it is set to a max of `100` by default and you may have more entries in your log output than that.
