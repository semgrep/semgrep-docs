...
By default, Semgrep places resource limitations on the size of files scanned and memory allocated.
...
Semgrep Code still performs [<Icon icon="file-lines" iconType="regular"/> interfile analysis](/semgrep-code/semgrep-pro-engine-intro#types-of-semgrep-code-analysis) on each module. If the modules are functionally separate, running separate scans shouldn't result in a reduction in findings.
</Info>
