...
Semgrep AppSec Platform [Policies](/docs/semgrep-multimodal/policies) can contain both individual rules and **rulesets**, which are curated groups of rules recommended for particular purposes. All organizations start with two rulesets: the `default` ruleset, which is a good starter pack for security teams, and the `comment` ruleset, which is a good starter pack for developers.
...
To change the default mode for a ruleset, follow the same process as for [adding a new ruleset to your policies](/docs/semgrep-multimodal/policies#add-rulesets-to-your-policies-from-the-registry) and select the desired default mode.

After adding the ruleset in the default mode, you can then [change any individual rule modes](/docs/semgrep-multimodal/policies#block-a-pr-or-mr-through-rule-modes) for rules that you prefer to keep in a different mode.
