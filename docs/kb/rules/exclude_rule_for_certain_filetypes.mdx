...
<Steps>
  <Step>
  Download the rule generating false positives from the [Registry](https://semgrep.dev/r).
  </Step>
  <Step>
  ...
  <Step>
  Exclude the target filetype in question from the rule through the `path` field: 
  ...
  Alter the scan command to still scan for the default configuration you have, with the following changes:
      ...
      &emsp;&emsp;i. Exclude the original noisy rule as articulated in the false positive reporting.
      
      &emsp;&emsp;ii. Include the new custom rule that excludes your target paths.
  </Step>
</Steps>
