...
<Frame>
![Example Semgrep YAML rule file with auto-complete](/images/kb/rules/using-semgrep-rule-schema-in-vscode/vscode-schema-autocomplete-example.png)
</Frame>
...
You can install the "YAML" extension authored by "Red Hat" directly in VS Code or by going to the Visual Studio Marketplace and installing it from there. In VS Code, go to the **Extensions** pane and search for `yaml`. This should yield the correct extension as the top result. However, please verify that you are installing the correct extension by ensuring it is the same as [this one](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml).

### Associate the Semgrep rule schema

Once the extension is installed, associate the Semgrep rule schema with the Semgrep YAML rule definitions you are working on in VS Code using one of the following methods:
...
<Frame>
![MacOS VS Code YAML extension settings](/images/kb/rules/using-semgrep-rule-schema-in-vscode/vscode-yaml-schemas.png)
</Frame>
...
<Frame>
![Example Semgrep YAML rule file with schema defined](/images/kb/rules/using-semgrep-rule-schema-in-vscode/vscode-yaml-schema-example-file.png)
</Frame>
