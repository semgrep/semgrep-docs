...
Generally, Semgrep recommends using the [<Icon icon="external-link" iconType="solid"/> GitLab merge request pipeline](https://docs.gitlab.com/ee/ci/pipelines/merge_request_pipelines.html) to receive MR comments. This method is used in the default [Semgrep GitLab config file](/semgrep-ci/sample-ci-configs/#sample-github-actions-configuration-file).

However, you can also receive comments through your own [<Icon icon="external-link" iconType="solid"/> GitLab runner](https://docs.gitlab.com/runner/) by setting the following variables in your CI job:
