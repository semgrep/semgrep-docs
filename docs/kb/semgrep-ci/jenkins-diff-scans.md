...
This guide provides two setup options: one for full Semgrep scans only, and one for full and diff-aware Semgrep scans. Full scans use a simple Pipeline project. Combining full and diff-aware scans uses a Multibranch Pipeline project. Both options use GitHub as the source code manager, with a repository whose default branch is `main`.
...
<Tabs
    ...
    values={[
    {label: 'Full scans', value: 'full-scan'},
    {label: 'Full and diff-aware scans', value: 'full-and-diff'},
    ]}
>

<TabItem value='full-scan'>
