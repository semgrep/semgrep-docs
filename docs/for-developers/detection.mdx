...
<Frame caption="A summary of differences between grep, linters, and Semgrep.">
![A summary of differences between grep, linters, and Semgrep.](/images/linters-semgrep-comparison-d74cc54168817728c80fe308fb3b9b37.png)
</Frame>
...
<AccordionGroup>
<Accordion title={"Simple syntax-based example: ban the use of `==` in JavaScript"}>
...
_**Figure**. Prevent type coercion in `==`. Click **<Icon icon="play" iconType="solid"/> Run** to view the findings._

This simple rule is accurate because it only requires the syntax defined in `pattern` to match, not the semantics. The **metavariables** $A and $B always evaluate to some value on the left and right hand side of the `==` operator, and that is all that matters, not the meaning of $A and $B themselves.
...
<Accordion title={"Complex syntax-based example: ban `console.log` in external or user-facing functions"}>
...
_**Figure**. Ban `console.log` in external-facing functions. Click **<Icon icon="play" iconType="solid"/> Run** to view the findings._
...
<Accordion title={"Semantic taint analysis: detecting unsanitized data from source to sink"}>
...
_**Figure**. Prevent possible cases of cross-site scripting due to unsanitized data. Click **<Icon icon="play" iconType="solid"/> Run** to view the findings._
