...
| **Provisionally ignored** | Rule-based Semgrep Code findings that Semgrep Multimodal's [Autotriage](/semgrep-multimodal/overview#autotriage) has flagged as likely false positives. You can change the status to **Ignored** if you agree with Multimodal's assessment. Otherwise, you can change the status to **To fix** if you disagree. <br /><br />**Note**: [AI-powered detection](/semgrep-code/ai-powered-detection-concepts) findings are **not** automatically set to **Provisionally ignored**. AI-powered detection findings that Multimodal autotriages as false positives remain **Open** and must be triaged manually. |
