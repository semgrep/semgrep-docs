...
Set the following environment variables to route Semgrep network traffic through a proxy:
...
Semgrep handles unusable proxy values as follows:

- An empty value is ignored. Semgrep prints a warning and the scan continues without a proxy.
- An invalid value stops the scan. The error message redacts credentials in the URL.
- A proxy URL without a scheme, such as `example.com:3128`, is given a scheme.
