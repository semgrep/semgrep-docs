...
The [Policies](/semgrep-appsec-platform/unified-policies/overview) feature manages rules, helps block PRs or MRs from entering production, and configures which findings are presented to developers.
...
You can also write custom rules and share these rules with the rest of your organization.
...
- Customize scans through rule selection in [policies](/semgrep-appsec-platform/unified-policies/overview)
...
- Policy-based workflows: Semgrep can perform workflow actions such as failing a CI job or leaving a PR comment based on user-defined [policies](/semgrep-appsec-platform/unified-policies/get-started) for all scans
