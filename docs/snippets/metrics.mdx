...
$ semgrep install-semgrep-pro   # → metrics enabled (requires log in)
```
...
When running `semgrep install-semgrep-pro`, which requires logging in, data on installations, including whether the installation was successful, is reported to Semgrep. If the installation failed, Semgrep receives a fixed identifier about the step that failed, such as `download-403` or `version-check-failed`, never a free-form error message.
