...
<dl>
    <dt>Policies header</dt>
        <dd>
            ...
            <ul>
             <li><strong>Validation State Policies</strong>, which lets you define how Semgrep handles findings that it categorizes as invalid or results in a validation error</li>
                    <li><i class="fa-solid fa-gear"></i> <b>Rule Modes</b> button where you can view <strong>rule modes</strong> and edit <strong>notifications</strong> for each rule mode. Rule modes define what <strong>workflow actions</strong> Semgrep Secrets performs when a rule detects a finding. For example, setting a rule's mode to <strong>Comment</strong> means that Semgrep posts PR or MR comments from findings generated by that rule. See <a href="#validation-state-policies">Validation state policies</a> for more information.</li>
            </ul>
        </dd>
    <dt>Filter pane</dt>
        <dd>
