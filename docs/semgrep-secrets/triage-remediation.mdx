...
Applying a **triage state** to the filtered findings based on the analysis in step 2.
   ...
   ii. Setting or retaining a finding as **Open**, **Reviewing**, or **Fixing** means that the finding is a true positive and needs to be fixed or resolved.
      ...
      &emsp;&emsp;a. Optional: You can [create a ticket in Jira](/semgrep-appsec-platform/jira) to assign a developer to fix findings.
</Step>
</Steps>
...
Findings for historical secrets always remain **Open**, because the secrets are identified after they've been removed from the code and Semgrep cannot determine whether they have been rotated or remediated. After you’ve addressed the issue, you can manually change the status to **Ignored**.
...
* You have [created a remediation policy](/semgrep-appsec-platform/unified-policies/get-started#create-a-remediation-policy) that lets you define the conditions under which Semgrep leaves a pull request comment. This customization enables you to:
...
Define the rules and the automations that the findings they produce should trigger using [Unified Policies](/semgrep-appsec-platform/unified-policies/overview).
</Note>
