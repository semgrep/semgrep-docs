...
- Findings from historical scans are generated through **Generic** (regex-based) rules only. To view these rules:
    - Navigate to **<a href="https://semgrep.dev/orgs/-/policies/secrets?analysis-method=generic" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> Semgrep AppSec Platform > Rules & Policies > Secrets</a>**.
    - Go to <Tooltip tip="Status indicating whether a secret poses an immediate threat. States include Confirmed valid, Confirmed invalid, Validation error, and No validator." cta="See full definition." href="/semgrep-secrets/glossary#validation-state">Validation state</Tooltip> policies > Global rule behavior. Click **Edit**.
