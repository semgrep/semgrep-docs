...
| **Ignored** | Findings that are ignored are present in the code, but have been labeled as unimportant. Ignore findings that are false positives or deprioritized issues. You can further filter findings with a status of **Ignored** by reason:  **False positive**, **Acceptable risk**, **No time to fix**. |
