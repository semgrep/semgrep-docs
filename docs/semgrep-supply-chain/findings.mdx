...
* **Fixed**: Findings that were detected in a previous scan but are no longer detected in the most recent scan of that same branch, usually because the dependency containing the vulnerability has been updated.
