...
In addition to the <a href="https://www.ntia.doc.gov/files/ntia/publications/sbom_minimum_elements_report.pdf" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> minimum elements that define an SBOM</a>, Semgrep provides additional metadata in the `vulnerabilities` field. Nested under the `vulnerabilities` field is a list of data objects describing a specific vulnerability. Each data object contains the following data fields:
