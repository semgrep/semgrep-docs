...
- **Remediate true positives**: If the finding is one that you must address, you can:
  ...
  - **Use Autofix**: [Supply Chain Autofix](/semgrep-supply-chain/autofix) can open a pull request (PR) or merge request (MR) that updates the vulnerable version in your repository and includes guidance on any breaking changes in the PR or MR description.
