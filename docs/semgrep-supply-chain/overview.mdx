...
- **Remediate true positives**: If the finding is one that you must address, you can:
  ...
  - **Use Upgrade Guidance (beta)**: If the remediation is to upgrade the package, [Upgrade Guidance](/semgrep-supply-chain/upgrade-guidance) uses program analysis and AI to analyze scan results and show whether you can safely update a vulnerable dependency to a fixed version.
  - **Use Autofix**: [Supply Chain Autofix](/semgrep-supply-chain/autofix) can open a pull request (PR) or merge request (MR) that updates the vulnerable version in your repository. If [Upgrade Guidance](/semgrep-supply-chain/upgrade-guidance) analysis has completed, the PR or MR description can also include guidance on potential breaking changes.
...
- [Get the status of an SBOM export job](https://semgrep.dev/api/v1/docs/#tag/SupplyChainService/operation/SupplyChainService_GetSbomExport)
