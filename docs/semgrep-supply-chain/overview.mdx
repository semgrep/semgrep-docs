...
- **Remediate true positives**: If the finding is one that you must address, you can:
  ...
  - **Use Upgrade Guidance**: If the remediation is to upgrade the package, **Upgrade Guidance** uses program analysis and AI to analyze scan results and show whether you can safely update a vulnerable dependency to a fixed version.
  - **Use Autofix**: Semgrep can open a pull request (PR) or merge request (MR) that updates the vulnerable version in your repository and includes guidance on any breaking changes in the PR or MR description.
