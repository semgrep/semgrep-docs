...
At this point, Semgrep displays the manifest files that it has used to determine dependency information and the dependencies included in each of the manifest files.

### View additional manifest files

By default, Semgrep only displays dependencies listed in a given project's first **10** manifest files. To load information from additional files:
...
* Semgrep Supply Chain can parse your manifest file. Refer to [Supported languages](/supported-languages) for a list of supported languages and manifest files.
