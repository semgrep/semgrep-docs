...
1. Using the **%%Dependency|dependency%%** search bar, enter the name of the dependency you are searching for.
...
1. Enter the dependency name and press **Enter** or **Return**. This returns a list of matches, but you can then filter your results further by version number:
   1. Click the name of your dependency to open the **%%Dependency|dependency%%** dialog:
...
2. Enter the **%%Dependency|dependency%%** name.
...
%%Dependency|dependency%% search provides the following filters, which correspond to the data points displayed by Semgrep about each dependency:

| Filter | Description |
| - | - |
| %%Dependency\|dependency%% | The name and version of the dependency. |
| Projects | The projects where the dependency can be found. |
| %%Transitivity\|transitivity%% | The relationship of the dependency to your codebase. |
