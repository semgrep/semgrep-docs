...
Supply Chain Autofix has no settings toggle. You can use it without enabling Upgrade Guidance.

For SCM permissions and read/write access, see [Autofix for Semgrep Code](/semgrep-code/triage-remediation/autofix#prerequisites) and [Grant read and write access](/semgrep-appsec-platform/scm-code-access). You also need [full Semgrep Supply Chain scans](/semgrep-supply-chain/set-up-and-configure) on at least one repository. For dependencies in a private registry, see [Connect a private registry to Semgrep](/kb/semgrep-supply-chain/connect-a-private-registry).

To open an Autofix PR or MR, see [Open an Autofix PR or MR with fixes](#open-an-autofix-pr-or-mr-with-fixes).
...
## Open an Autofix PR or MR with fixes
