...
- [Fix findings that are true positives](#fix-findings-that-are-true-positives) by:
    ...
    - [Using Upgrade Guidance to update the dependency to a safe version](#upgrade-guidance).
    - [Using Autofix to update the dependency to a safe version](#autofix).
...
See [Upgrade Guidance and Autofix](/semgrep-supply-chain/triage-and-fix/upgrade-guidance-and-autofix) for setup and usage details.

### Autofix

**Autofix** can open pull requests and merge requests that update the version used by your project and guide you through any breaking changes the update introduces.

See [Enable Autofix](/semgrep-supply-chain/triage-and-fix/upgrade-guidance-and-autofix#enable-autofix) and [Open an Autofix PR or MR with fixes](/semgrep-supply-chain/triage-and-fix/upgrade-guidance-and-autofix#open-an-autofix-pr-or-mr-with-fixes).
