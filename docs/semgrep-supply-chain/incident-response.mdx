...
When Semgrep declares an incident, projects that use Semgrep Managed Scans for Supply Chain scanning and have dependencies in an affected ecosystem are rescanned automatically with the incident's rules. See [Supply Chain incident notifications](/semgrep-supply-chain/incident-notifications) for details.

You can also run full scans on potentially affected repositories manually:
