...
Autofix is enabled for all customers. There is no settings toggle. You choose when to use Autofix by selecting **Open Autofix PR** from a finding's **Fix** menu or by calling the API.
...
- [Enable Semgrep Supply Chain scans](/semgrep-supply-chain/set-up-and-configure) and run a full scan on a connected repository with an eligible finding.
- The finding has a fixed version to upgrade to.
- The project uses a supported ecosystem: JavaScript (`npm`) or Python (`pip`, `pip-tools`, `pipenv`, `poetry`, `uv`) on GitHub Cloud or GitLab Cloud.
- Grant the SCM roles, scopes, and app permissions Autofix needs for your SCM. See [SCM permissions](/deployment/prepare/scm-permissions#permissions) for the **Autofix** row for your provider and [Grant read and write access](/semgrep-appsec-platform/scm-code-access) to configure your connection. For GitHub API and security review details, see [Autofix permissions](/deployment/prepare/scm-permissions#autofix-github-permissions).
  - **GitHub users** must install the private GitHub app for Semgrep with [Contents: Read and write](/semgrep-appsec-platform/scm-code-access#grant-code-access-to-semgrep-with-a-private-github-app) permissions.
