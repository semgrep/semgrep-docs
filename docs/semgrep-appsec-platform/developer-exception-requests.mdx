...
<Tabs>
    ...
    **For requests initiated in Semgrep AppSec Platform:** View the finding to see its status. If your request is approved, the status updates to **Ignored**. Otherwise, the status is updated to **To fix**.
    ...
    <Warning>
        Exception requests override the **Default developer triage permissions** setting in Semgrep AppSec Platform under **Settings > Global**. For example, even if that setting is turned off and developers can't triage findings, developers can still request exceptions for findings that a policy definition requires exceptions to ignore.
    </Warning>
