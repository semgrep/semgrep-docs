...
- Findings from archived projects are not included in the **Reporting** page or analytics.
...
| Total ignored  | Total number of **Ignored** findings during the time period that **remained** ignored until the end of the time period. **Ignored** findings include those with a status of **Provisionally ignored**. |
