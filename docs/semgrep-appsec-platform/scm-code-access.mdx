...
Use this page for **how** to grant Semgrep read or write access to source code on an existing SCM connection: update a private GitHub App, or create and enter an access token.

Some Semgrep features, including [Semgrep Managed Scans](/deployment/managed-scanning/overview), [Semgrep Multimodal](/semgrep-multimodal/overview), and [Autofix](/semgrep-code/triage-remediation/autofix), require Semgrep to read repository contents. Autofix also requires write access.

For **what** roles and scopes each feature needs, see [SCM permissions](/deployment/prepare/scm-permissions). That reference covers GitHub App permissions, personal access token details, PR or MR comments, and sign-in scopes.

The following table shows the minimum scopes required for each SCM. For GitHub personal access tokens, including classic vs fine-grained details, account roles, and connection validation, see [GitHub personal access tokens](/deployment/prepare/scm-permissions#github-personal-access-tokens).
...
| GitHub.com and GitHub Enterprise Server | `Contents: Read` | `Contents: Read and write`<br />`Pull requests: Read and write` |
...
If you connected your repositories using an access token, follow these steps to grant code access to Semgrep.
...
Create a GitHub personal access token (PAT), then add it to Semgrep. Semgrep recommends a **fine-grained** PAT. **Classic** PATs remain supported as well.

For account roles, fine-grained and classic scopes, and what Semgrep checks when you save or **Test** a connection, see [GitHub personal access tokens](/deployment/prepare/scm-permissions#github-personal-access-tokens).

<Tabs>
  <Tab title="Fine-grained PAT (recommended)">
...
GitHub automatically includes **Metadata: Read** when you grant a repository permission. You do not set Metadata separately.
</Step>
<Step>
...
<Step>
Click **Save**. Optionally click **Test** to confirm the connection.
</Step>
</Steps>
...
Once saved, Semgrep has code access to your GitHub repositories.
  ...
  <Tab title="Classic PAT">

Use a classic PAT only when fine-grained tokens are unavailable or not permitted by policy. Choose scopes from the [classic scopes column](/deployment/prepare/scm-permissions#github-personal-access-tokens) for your use case. For most private-repository deployments, select `repo`.

<Steps>
<Step>
Navigate to the GitHub classic personal access token settings page: <code>https://github.com/settings/tokens</code>. GitHub Enterprise Server users must replace the <code>https://github.com</code> base URL with the base URL of their GitHub Enterprise Server instance.

You can also open **Settings > Developer settings > Personal access tokens > Tokens (classic)**.
</Step>
<Step>
Click **Generate new token** and choose **Generate new token (classic)**.
</Step>
<Step>
Enter a note for the token and set an expiration date according to your organization's token rotation policy.
</Step>
<Step>
Select the classic scopes required for your use case. For most private-repository deployments, select `repo`.
</Step>
<Step>
Click **Generate token** and copy its value. GitHub only shows the token value once.
</Step>
<Step>
If the organization uses SAML single sign-on, open the token's **Configure SSO** options and authorize the token for that organization.
</Step>
<Step>
Return to Semgrep AppSec Platform, and go to [**Settings > Source code managers**](https://semgrep.dev/orgs/-/settings/source-code).
</Step>
