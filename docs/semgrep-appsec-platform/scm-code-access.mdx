...
Use this page for **how** to grant Semgrep code read or write access on an existing SCM connection: update a private GitHub App, or create and provide an access token.
...
Create a GitHub personal access token (PAT), then add it to Semgrep. Use a PAT when the connection uses a token instead of a GitHub App. Semgrep recommends a **fine-grained** PAT. **Classic** PATs remain supported when your organization requires them.
...
Use a classic PAT only when fine-grained tokens are blocked by organization policy. Choose scopes from the [classic scopes column](/deployment/prepare/scm-permissions#github-personal-access-tokens) for your use case. For most private-repository deployments, select `repo`.
