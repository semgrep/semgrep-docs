...
1. Create a Semgrep account connection with each GitLab group by signing in to your Semgrep account, navigating to **Settings > Source code managers**, and adding an entry for each group. If you want to onboard a parent group and all of its subgroups, you only need to add an entry for the parent group.
