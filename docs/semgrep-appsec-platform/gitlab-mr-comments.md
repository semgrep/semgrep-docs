...
To enable MR comments, you need to connect your GitLab organization to Semgrep AppSec Platform:

1. In your Semgrep AppSec Platform account, click **Settings > Source code managers**.
2. Click **Add connection** and select GitLab.
3. Create a GitLab personal access token (PAT) with `api` scope:
   1. In GitLab, go to [<i class="fas fa-external-link fa-xs"></i> Profile > Access Tokens](https://gitlab.com/-/profile/personal_access_tokens)
   2. Add a token with `api` scope
   3. Copy the generated token
4. In the Semgrep connection form:
   1. Enter your GitLab group slug
   2. Paste the PAT you created
   3. Click **Connect**

Once connected, Semgrep will automatically be able to post comments on your merge requests without any additional CI/CD configuration.
