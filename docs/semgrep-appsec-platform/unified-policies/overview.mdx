...
Unified Policies allow you to choose the rules and rulesets used for Semgrep scans to increase the breadth and depth of your scan coverage or remove noise from scans. You can also define what happens to a finding after identification, such as whether a finding is monitored, generates a pull request (PR) or merge request (MR) comment, or blocks a PR or MR.

With Unified Policies, there are two types of policy definitions available to you:
