...
To create a policy that defines the automated responses to security findings:<br/><br/>
   ...
   <Info>
   To use the **Send a Slack message** action, first make the channel available by running `/semgrep_add_channel` in that Slack channel. See [Receive Slack notifications](/semgrep-appsec-platform/slack-notifications#unified-policies).
   </Info>
