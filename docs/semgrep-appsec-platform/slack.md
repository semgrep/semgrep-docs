...
You can select the channels in your Slack workspace receive findings. You can also choose to receive findings only for certain repositories (projects) or **Rule Modes**. For example, you can choose to receive notifications only for findings generated by rules from the Blocking Rule Mode.
