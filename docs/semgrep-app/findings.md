...
| **Fixed** | Fixed findings are findings that were detected in a previous scan of a particular branch, but no longer trigger a match in the most recent scan of that same branch. The rule that detected the finding and the code that triggered the match must both be active in the most recent scan. Findings can be fixed by either fixing the code such that the rule no longer matches it, or editing the rule such that it no longer matches the code. |
