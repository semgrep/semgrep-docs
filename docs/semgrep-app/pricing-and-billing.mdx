...
Billing depends on the number of **seats**. Any organization member who has made a commit in a project within the past 30 days is considered a **contributor** and takes up a seat.

Members who do **not** make any commits on the code do not need a seat and are not billed. Semgrep Team tier features apply to the entire organization, even if not all members are billed for a seat.
...
| -------  | --------- | ------- | -------- |
| **Member limit** | 20 seats | ♾️  unlimited | ♾️  unlimited |
...
* If your organization exceeds the member limit:
    * Only the 20 most recent contributors can view findings from scans run by Semgrep.
...
Within your team or organization, assess the number of members that make commits. That will determine the number of seats needed for purchase.

For example, if a project has 4 unique contributors who create commits during the billing period and Semgrep runs on their merge or pull requests, only 4 seats are required even if the organization has a total of 10 members. If these unique contributors commit to many projects within the same organization, they will only be counted once, so no additional seats are required.
