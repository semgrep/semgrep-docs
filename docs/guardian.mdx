...
Guardian uses different rules depending on the environment:

* **Claude Code remote server (default):** scans with the [Guardian default ruleset](https://semgrep.dev/p/guardian-default). This is not configurable today, but we're working on adding support for custom rules on the remote server. If you want this, please reach out in Semgrep's `#mcp` [Slack community](https://go.semgrep.dev/slack).
* **Claude Code local and all other IDE integrations:** scans with the rules enabled on your Policy within your Semgrep organization.
