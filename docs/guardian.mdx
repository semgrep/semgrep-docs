...
If you are already logged in through `semgrep login`, Guardian can use those credentials from `settings.yml`. For the remote Claude Code plugin, Guardian checks for OAuth credentials in `guardian.yml` first. If an OAuth session is present, Guardian uses it instead of any API token in `settings.yml`.
