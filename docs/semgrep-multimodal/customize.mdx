...
When Semgrep Multimodal is enabled, you can use [Autofix](/semgrep-code/triage-remediation/autofix) for Semgrep Code findings. Autofix (beta) opens a draft pull request or merge request with proposed fixes. There is no separate organization-level toggle to enable or disable Autofix for Semgrep Code.
