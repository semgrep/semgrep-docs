...
To receive AI-generated code changes for Semgrep Code findings, enable [Autofix](/semgrep-code/triage-remediation/autofix). Autofix (beta) opens a pull request or merge request with proposed fixes.
...
<Note>
The models Multimodal requires must be available for on-demand inference in the selected region. Because of that requirement, regions outside the US are not currently supported. In those regions, AWS offers the required Anthropic Claude models only through cross-region inference profiles rather than as direct on-demand models.
</Note>
