...
Multimodal **Suggested fix** includes suggestions on how to fix true positives. Suggested fix does **not** include inline code diffs. Perform the following to enable it:
...
<Steps>
  ...
  <Step>
  *Optional*: Select a **confidence level** in the drop-down box. This value determines the quality of Suggested fix guidance. For example, if you select lower confidence, Semgrep Multimodal suggests a fix even when the guidance quality is lower.
  </Step>
</Steps>
...
Semgrep recommends setting a low confidence level since even imperfect guidance may be useful starting points for triage and remediation.
</Tip>

## Autofix

To receive AI-generated code changes for Semgrep Code findings, enable [Autofix](/semgrep-code/triage-remediation/autofix). Autofix opens a draft pull request or merge request with proposed fixes.
...
<Steps>
  ...
  <Step>
  Click the **Autofix** <Icon icon="toggle-large-on"/> toggle to enable this feature.
  </Step>
</Steps>
...
Memories allow admins to tailor Multimodal's Suggested fix guidance to their organization's standards and defaults. You can provide feedback by adding custom instructions whenever Multimodal provides a Suggested fix.
...
<Steps>
  ...
  <Step>
  Multimodal regenerates the Suggested fix guidance to reflect the instructions you provided.
  </Step>
</Steps>
