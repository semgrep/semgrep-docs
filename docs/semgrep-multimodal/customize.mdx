...
<Steps>
  ...
  <Step>
  Click the **Suggested fix** <Icon icon="toggle-large-on"/> toggle to enable this feature.
  </Step>
  <Step>
...
<Steps>
  ...
  <Step>
  Click the **Weekly priority emails** <Icon icon="toggle-large-on"/> if it is not yet enabled.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
  Click the **Noise filter for Code PR/MR comments** <Icon icon="toggle-large-on"/> if it is not yet enabled.
  </Step>
  <Step>
  Select whether you want to enable PR or MR comments for false positive findings:
   1. **Don't leave a PR/MR comment**: Hide Semgrep's comments on findings that are likely to be false positives. These findings are available for security review on the [**Code > Pre-production backlog** page](https://semgrep.dev/orgs/-/findings?tab=open&last_opened=All+time&backlog=preprod). Comments still appear for rules in remediation policies where the Semgrep action is to leave comments if the conditions of the policy are met.
...
<Steps>
  ...
  <Step>
  Navigate to [<Icon icon="gear"/> **Rules & Policies > Memories**](https://semgrep.dev/orgs/-/memories).
  </Step>
  <Step>
  Click **New Memory**.
  </Step>
...
<Steps>
  ...
  Identify the specific instance of **Multimodal's Suggested fix** that you want to modify. This can be found on the finding details page or in the PR or MR comment.
   - If Multimodal used existing memories to generate the guidance, you can click on **Referenced <span className="placeholder">X</span> memories while writing this guidance** to see the memories used.
  </Step>
  <Step>
...
<Steps>
  ...
  <Step>
  Navigate to [<Icon icon="gear" iconType="solid"/> **Rules & Policies > Memories**](https://semgrep.dev/orgs/-/memories).
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
  Identify the memory you would like to delete, then click the <Icon icon="trash"/>**icon** to remove the memory.
  </Step>
</Steps>
...
You can also use your own API keys or accounts for the following providers:
...
<Steps>
<Step>
Sign in to [Semgrep AppSec Platform](https://semgrep.dev/login) and navigate to [<Icon icon="gear" iconType="solid"/> **Settings > Global**](https://semgrep.dev/orgs/-/settings/general).
</Step>
<Step>
...
<Steps>
<Step>
Sign in to [Semgrep AppSec Platform](https://semgrep.dev/login) and navigate to [ <Icon icon="gear"/> **Settings > Global**](https://semgrep.dev/orgs/-/settings/general).
</Step>
<Step>
Click the <Icon icon="gear" iconType="solid"/> **icon** next to **AI provider**.
</Step>
...
2. To configure Semgrep to use Azure OpenAI:
   1. Sign in to [Semgrep AppSec Platform](https://semgrep.dev/login) and navigate to [<Icon icon="gear" iconType="solid"/>**Settings > Global**](https://semgrep.dev/orgs/-/settings/general).
   2. Click the <Icon icon="gear" iconType="solid"/> **icon** next to **AI provider**.
   3. Select **Azure OpenAI**.
   4. Paste the **Target URI** you copied from Azure into **Your Azure OpenAI Endpoint**. The endpoint must include the full path and API version query parameter. The expected format is:
...
<Warning>
If Semgrep cannot save your endpoint and API key, confirm that the endpoint includes both `/openai/deployments/DEPLOYMENT_NAME/chat/completions` and the `?api-version=` query parameter. Pasting only the base resource URL (for example, `https://my-resource.openai.azure.com`) fails with a missing configuration error.
</Warning>

<Note>
Semgrep supports one Azure OpenAI deployment at a time. Because each Azure deployment serves a single model, that model is used for all Semgrep Multimodal features, including noise filtering. If you want a different model, deploy it in Azure and then repeat the steps above with its Target URI and API key, which replaces your existing configuration.
</Note>

Azure OpenAI is not available for [AI-powered detection scans](/semgrep-code/ai-powered-detection-concepts) or [Agentic Workflows](/workflows/overview). Those features require a Semgrep-managed OpenAI or Amazon Bedrock provider.
