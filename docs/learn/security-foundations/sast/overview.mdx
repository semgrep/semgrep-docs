...
- **Support many programming languages and frameworks** used during development, this includes core programming languages, infrastructure as code, and scripts.
...
<Frame>
![dataflow example](/images/sast-dataflow-b2405db9f118fb42f013868d7b98b0e2.png)
</Frame>
...
Being able to write custom rules helps with edge-cases where internal knowledge is needed to be able to accurately report if something is a security vulnerability or passes the code smell test. For example:
...
- You have created a custom HTTP request library that you want to track as a source if any request parameters enter dangerous sink locations that lead to SQL Injection or SSRF.
