...
Sometimes in large codebases, legacy code, or unsafe patterns persist in documentation that may have been used as a reference.
...
There are DB compliant libraries that can be used with a variety of databases including sqlite, postgres, mysql, and more.
...
A success or failure in a query can leak information.
...
This is the most reliable way to prevent SQLi. Instead of building the SQL string manually, placeholders are used and the values are passed separately. This separates code from data and lets the library properly check for and escape any malicious strings. 

This is a Python example that lets the `execute` function sanitize the query string that was passed with an HTTP request:
