...
There is also potential for [code injection](/learn/vulnerabilities/code-injection), where instead of executing a command, source code is included in the configuration to run. Command and code injection have a common security vulnerability theme: untrusted user input included in GitHub Actions workflows can lead to exfiltrating tokens, compromising infrastructure, or creating releases to distribute malware.
...
For any secrets configured, limit their access from org-wide level whenever it is not necessary. Branch protection rules can also be effective at limiting permissive settings that can be detected before being exploited.
