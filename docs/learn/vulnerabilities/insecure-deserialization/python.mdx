...
In this article, we explain the fundamentals of serialization in the context of Python. Then, we illustrate the most common ways insecure deserialization is exploited, and show you how to detect these patterns in your own code. Finally, we provide some practical recommendations to avoid the risks.
...
We highly recommend using automated tools to verify compliance, since many more libraries use deserialization powered by libraries such as `pickle` under the hood. Running Semgrep regularly as part of your continuous integration pipeline can highlight insecure deserialization patterns before they reach production.
