...
* Your repository's `.gitignore` file (if it exists)
  - If a file present in the repository's `.gitignore` is currently committed and tracked, Semgrep will scan that file.
...
* An `:include ...` directive is added, which allows another file to be included in the ignore pattern list; typically this included file would be the project `.gitignore`. No attempt at cycle detection is made.
  - If a file that is already committed and tracked is in the project `.gitignore,` including `.gitignore` in `.semgrepignore` will not exclude that file. Add the file directly to the `.semgrepignore` to exclude it.
...
The organization-wide `nosemgrep` setting is in **beta**.

`nosemgrep` comments are inline comments that allow you to ignore the specified blocks of code. This functionality works across all supported languages.
...
To use `nosemgrep` comments:

1. Enable `nosemgrep` comments in Semgrep AppSec Platform:
    1. Sign in to [<i class="fas fa-external-link fa-xs"></i> Semgrep AppSec Platform](https://semgrep.dev/login).
    1. Go to **Settings > Global** and ensure that the **Allow inline comments to disable Semgrep** toggle is enabled.
1. Go to your code file where there are blocks of code you want Semgrep to ignore. Define an **inline comment**, followed by the word `nosemgrep`, at either the **first line** or **the line preceding** the potential match. Semgrep ignores all rule pattern matches.
...
To ignore blocks of code for a **specific rule**, enter its `rule-id` as follows: `nosemgrep: RULE_ID`. To ignore **multiple rules**, use a comma-delimited list. `rule-ids` must be referenced with their namespace.
...
<Tabs>
  ...
  <Tab title="JavaScript">
    For this example, assume that the rules are stored in a `configs` subdirectory:
    ```javascript
...
<Info>
**NOTE**: Previous annotations for ignoring code inline, such as `nosem`, are deprecated.
</Info>

## Manage rules and rulesets using Semgrep AppSec Platform

If you're a Semgrep AppSec Platform user, you can add and remove rules and rulesets and turn them on and off as needed by [modifying your organization's detection policies](/semgrep-appsec-platform/unified-policies/get-started#manage-detection-policies).
