...
[Autofix](/semgrep-code/triage-remediation/autofix) is supported on all source code managers in the table above at supported plan tiers (see footnotes † and *). Autofix is not yet available when Semgrep connects to your SCM through the [Semgrep Network Broker](/semgrep-ci/network-broker). 
