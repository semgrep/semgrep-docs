...
[Autofix](/semgrep-code/triage-remediation/autofix) is supported on all source code managers in the table above at supported plan tiers (see footnotes † and *). When Semgrep connects to your SCM through the [Semgrep Network Broker](/semgrep-ci/network-broker), Autofix requires Network Broker `v0.45.2` or later with `allowCodeAccess` set to `true`. See [Use Semgrep Network Broker with Autofix](/semgrep-ci/network-broker#use-semgrep-network-broker-with-autofix).
