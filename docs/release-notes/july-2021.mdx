...
- New iteration of taint-mode that allows to specify sources/sanitizers/sinks using arbitrary pattern formulas. This provides plenty of flexibility. Note that we break compatibility with the previous taint-mode format, e.g., - source(...) must now be written as - pattern: source(...).
...
- The size of the semgrep-core binary is now 95 MB (was 170 MB in v0.58.0) and a smaller Docker image (from 95 MB to 40 MB)
