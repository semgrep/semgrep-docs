...
- Fixed an issue where agreeing with an autotriage verdict incorrectly marked findings as ignored. Findings are now only auto-ignored when the user assigns it as a **False Positive**.
...
<CardGroup>
  <Card title="Semgrep Managed Scans doesn't run for pull requests in GitHub merge queues" icon="code-branch" href="/kb/semgrep-appsec-platform/no-runs-in-github-merge-queues" horizontal />
  <Card title="Why does the Projects page display a different dependency count from the Dependencies page?" icon="bar-chart" href="/kb/semgrep-appsec-platform/dependency-count-differ-platform" horizontal />
</CardGroup>
...
<CardGroup>
  <Card title="1.143.0" icon="code-branch" href="https://github.com/semgrep/semgrep/releases/tag/v1.143.0" horizontal />
  <Card title="1.144.0" icon="code-branch" href="https://github.com/semgrep/semgrep/releases/tag/v1.144.0" horizontal />
</CardGroup>