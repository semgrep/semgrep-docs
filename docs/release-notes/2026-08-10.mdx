...
- Fixed an issue where [historical scanning](/semgrep-secrets/historical-scanning) on Semgrep Managed Scans examined only a single commit instead of Git history. The setting now applies before the repository is cloned.
- Fixed an issue where requesting a historical secrets scan for an organization without Semgrep Secrets caused the entire scan to fail, including Code and Supply Chain results.
