...
- CI scans that use Semgrep Pro Engine now run intrafile and interprocedural taint analysis by default in differential scans (such as PR or MR scans). Note that cross-file analysis is not run in differential scans for performance reasons.
