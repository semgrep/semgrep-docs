...
- The `semgrep scan` command is now more resilient to failures when fetching a configuration file (config) from Semgrep servers.
    - If it can't fetch a config from Semgrep servers it will use backup infrastructure to fetch the most recent successful config for that customer's environment. ([#8459](https://github.com/semgrep/semgrep/pull/8459/))
...
- **JavaScript (JS) or TypeScript (TS) taint mode:** fixed a bug introduced in 1.33.1 that had the side-effect of hurting performance of taint rules on JS or TS repositories that used destructuring in a function's formal parameters.
...
- Added a **<Icon icon="clipboard" iconType="regular"/> Copy rule** button within the rule popup.
...
- Added section on how a future [change in a Semgrep Supply Chain rule affects scan behavior](/semgrep-supply-chain/set-up-and-configure/#schedule-scans).
