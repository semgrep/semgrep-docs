...
- You can now export your findings in CSV format. Semgrep can export up to 10,000 most recent findings. For findings greater than 10,000, use the [API](/api-reference/v1/Introduction). See [Export findings](/semgrep-code/findings#export-findings) for more information.
...
example, the following code is TypeScript, as opposed to pure JavaScript, because it uses decorators on function parameters:
  ...
  function $FN(..., @Bar(...) $X, ...) { ... }
  ```
- C#: Patterns such as `new $T(...)` now match C# target-typed new expressions such as `new ()`.
