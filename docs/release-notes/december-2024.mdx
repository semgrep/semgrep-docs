...
- Semgrep OSS has been renamed to **Semgrep Community Edition (CE)**. Semgrep CE remains free, with 2800+ rules and no login required. See also <a href="https://semgrep.dev/blog/2024/important-updates-to-semgrep-oss/" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> Important updates to Semgrep OSS</a> in the Semgrep blog.
- Rules authored and maintained by Semgrep, Inc. are now licensed under <a href="https://semgrep.dev/legal/rules-license/" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> Semgrep Rules License v.1.0</a>. These rules are available only for internal, non-competing, and non-Software-as-a-Service (SaaS) contexts.
