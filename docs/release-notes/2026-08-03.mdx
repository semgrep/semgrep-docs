...
- **Projects** page findings counts now include findings in **Open**, **Reviewing**, and **To fix** statuses. Counts on the page increase to reflect the additional triage states, and a callout on the Projects list explains the new count semantics.
- The unified policies migration banner now includes the migration schedule: Semgrep will start migrating some organizations to unified policies on August 24, 2026, and will sunset the current version of Policies on November 1, 2026. Organizations that do not want to migrate yet can contact Support.
...
- The in-app API reference pages at `/api/v1/docs` and `/api/v2/docs` now redirect to the [API reference](/api-reference/v1/Introduction) on docs.semgrep.dev.
