...
- Semgrep Playground now displays its version number. To see the exact version that Semgrep Playground uses, click the <Icon icon="ellipsis-vertical" iconType="regular"/> three-dot button, and then see the version number after the <Icon icon="code-commit" iconType="solid"/> icon.
    <Frame>
    ![Semgrep Playground version number.](/images/semgrep-app-latest-version-4c1c4995ad453ca03a44bf2dfe29d4c4.png)
    </Frame>
...
- [Scanning open source dependencies](/semgrep-supply-chain/set-up-and-configure) - Walks the user through setting up Semgrep Supply Chain scans and how Semgrep performs reachability analysis.
