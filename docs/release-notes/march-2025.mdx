...
- Added the capability to delete projects through the Semgrep API. Deleting a project also deletes all of its findings. Refer to the [API documentation](/api-reference/Introduction).
- You can now view the `cwe_names` and `owasp_names` for findings fetched through the Semgrep API. See the [API documentation](/api-reference/Introduction).
...
- Added <a href="https://semgrep.dev/llms.txt" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> `llms.txt`</a>.
...
- Added the ability to [download SBOM exports using the Semgrep API](/api-reference/supplychainservice/create-a-new-sbom-export-job).
