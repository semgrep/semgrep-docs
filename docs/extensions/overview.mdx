...
Prevent secrets or security issues from entering your Git source control history by running Semgrep as a <a href="https://pre-commit.com/" target="_blank" rel="noopener noreferrer"><Icon icon="external-link" iconType="solid"/> pre-commit</a> hook. See [`pre-commit` documentation](/extensions/pre-commit) for details.
