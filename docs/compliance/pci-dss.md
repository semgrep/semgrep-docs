...
**Important:** Cardholder data should never exist in code repositories. Use designated test card numbers for testing. If no cardholder data exists in your code, PCI DSS does not apply to your SAST scanning.
...
For deployment guidance: CLI and on-premises CI/CD keep code in customer-controlled infrastructure. Cloud CI/CD and Semgrep Managed Scans process code repositories only (where cardholder data should not exist). If cardholder data is present in the code, verify that your deployment option meets your PCI scope requirements.
