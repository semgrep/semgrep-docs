...
Having permission to scan is separate from whether you can open the **Projects** page. Members may still have scan permission so they can run local CLI scans and upload results to the organization, even when they cannot open **Projects** and therefore cannot use **Run a new scan** in the AppSec Platform UI.

With **Teams** enabled, **Members** who are assigned to a team can view the projects for that team on the **Projects** page. Viewing those projects does not grant the ability to start an on-demand scan. Starting an on-demand scan with **Run a new scan** requires a role with scan permission, such as **Admin**. Members without that permission can still run local CLI scans and upload results.
