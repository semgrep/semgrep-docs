...
| Semgrep Code is too noisy.         | Enable [cross-file (interfile) analysis](/semgrep-code/semgrep-pro-engine-intro) or remove rules and rulesets from your [detection policies](/semgrep-appsec-platform/unified-policies/overview).  |
| I want my developers to see certain security issues in their pull request or merge request.         | Create a [remediation policy](/semgrep-appsec-platform/unified-policies/get-started#create-a-remediation-policy) where the action is to comment on a pull request or merge request.  |
...
| I want to enforce my organization's coding standards. | Write a [custom rule](/writing-rules/overview) and [add it to your detection policies](/semgrep-appsec-platform/unified-policies/get-started#manage-detection-policies). |
...
<CardGroup>
  <Card title="Semgrep release notes feed" icon="rss" href="https://docs.semgrep.dev/whats-new/rss.xml" horizontal />
