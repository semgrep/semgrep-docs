...
<Warning>
Once a SCIM directory is activated for a deployment, user management is locked in Semgrep AppSec Platform. Only teams created through SCIM are locked. Make those user and team changes through your identity provider.
</Warning>
...
If a user belongs to multiple role-suffixed groups for the same team, Semgrep assigns the highest-privilege team role among those groups (**Manager**, then **Member**, then **Read-only**).
...
- Once SCIM is active, you can't manage SSO users or SCIM-created teams through Semgrep AppSec Platform. You can still manage teams that weren't created through SCIM.
