...
| Semgrep Code is too noisy.         | Enable [cross-file (interfile) analysis](/semgrep-multimodal/semgrep-pro-engine-intro) or remove rules and rulesets through the [Policies page](/semgrep-multimodal/policies).  |
| I want my developers to see certain security issues in their pull request or merge request.         | Configure [Comment mode](/semgrep-multimodal/policies#block-a-pr-or-mr-through-rule-modes) in the Policies page.  |
