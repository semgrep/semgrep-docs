...
Semgrep recommends connecting GitHub with the public [`semgrep-app`](https://github.com/apps/semgrep-app) and, for code access features, a [private Semgrep GitHub App](#private-github-app-permissions). Some organizations instead use a [personal access token (PAT)](#github-personal-access-tokens). The table below covers the GitHub App path. For PAT roles and scopes, see [GitHub personal access tokens](#github-personal-access-tokens).
...
Semgrep recommends connecting GitHub with a GitHub App. Some organizations use a personal access token (PAT) instead. Use the tabs below for the connection method that applies to you.
...
Semgrep uses two GitHub Apps on GitHub.org and GitHub Enterprise:

| App | Typical name | What it is for |
| :--- | :--- | :--- |
| **Public** | [`semgrep-app`](https://github.com/apps/semgrep-app) | Sign-in, connecting your organization, listing repositories and members, PR comments, and CI onboarding such as writing workflow files. |
| **Private** | **Semgrep Code - <span className="placeholder">YOUR_ORG_NAME</span>** | Code access for [Managed Scans](/deployment/managed-scanning/overview), [Semgrep Multimodal](/semgrep-multimodal/overview), and [Autofix](/semgrep-code/triage-remediation/autofix). Your organization creates and installs this app. |

You usually need both apps for Managed Scans and AI-assisted features. The sections below describe the permissions each path requests.
...
### Private GitHub App permissions {#private-github-app-permissions}

<div id="permissions-for-managed-scans-and-ai-features" />

Your organization creates a private GitHub App named **Semgrep Code - <span className="placeholder">YOUR_ORG_NAME</span>**. That app provides repository **Contents** access and related permissions for:
...
#### Autofix permissions and API usage {#autofix-github-permissions}

<div id="autofix-permissions" />

Use this section for security reviews of Autofix on GitHub, such as determining which GitHub operations require **Contents: Read and write**.

Autofix uses your **private Semgrep GitHub App** with the repository permissions below. See [Grant code access](/semgrep-appsec-platform/scm-code-access) to configure **Contents: Read and write** under **Repository permissions > Contents** on the private app.
...
| Code access | Same as above; user must be able to read the target repositories | <ul><li>All **Connect or Test** requirements</li><li>**Contents: Read**</li></ul> | Same as **Connect or Test**: `repo` or `public_repo`. Classic `repo` includes content read. | [Grant code access](/semgrep-appsec-platform/scm-code-access#grant-code-access-to-semgrep-for-token-based-scm-connections) |
...
- [GitHub Managed Scans](/deployment/managed-scanning/github): install public and private Semgrep GitHub Apps for Managed Scans
