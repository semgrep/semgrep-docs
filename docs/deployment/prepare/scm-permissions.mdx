...
To **configure** code read or write access on a connection, see [Grant code access](/semgrep-appsec-platform/scm-code-access). That page covers updating a GitHub App or access token.
...
Semgrep recommends connecting GitHub with the public [`semgrep-app`](https://github.com/apps/semgrep-app) and, for code access features, a [private Semgrep GitHub App](#permissions-for-managed-scans-and-ai-features). Some organizations instead use a [personal access token (PAT)](#github-personal-access-tokens). The table below covers the GitHub App path. For PAT roles and scopes, see [GitHub personal access tokens](#github-personal-access-tokens).
