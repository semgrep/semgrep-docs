...
<Step>
Click the login URL provided, or copy and paste it into your browser's address bar. You are taken to your web browser to complete the login process.
</Step>
<Step>
...
- **For organization orgs**: A local repository scan does **not** overwrite findings records of its remote counterpart. However, if two members have both cloned the same local repository, such as `RepoA`, and both send local `RepoA` findings, one set of findings may overwrite the other unintentionally. This is because orgs can have more than one team member, but all local scans are sent to the same project slug.
