...
// Testing branch-specific configuration
const config = {
  indexName: 'docs_testing',
  startUrls: [
    'https://semgrep-docs.netlify.app/docs/release-notes',
    'https://semgrep-docs.netlify.app/docs/rule-updates', 
    'https://semgrep-docs.netlify.app/docs'
  ],
  sitemapUrls: ['https://semgrep-docs.netlify.app/docs/sitemap.xml'],
  stopUrls: [
    'https://semgrep-docs.netlify.app/docs/tags/*',
    'https://semgrep-docs.netlify.app/docs/category/*'
  ],
  selectors: {
    lvl0: {
      selector: '.breadcrumbs > li:nth-child(2) span.breadcrumbs__link',
      global: true,
      defaultValue: 'Semgrep documentation (Testing)'
    },
    lvl1: 'article h1',
    lvl2: 'article h2',
    lvl3: 'article h3',
...
class StagingMeilisearchIndexer {
  constructor() {
    const host = process.env.MEILISEARCH_STAGING_HOST_URL || 'https://staging-meilisearch.semgrep.dev';
    const apiKey = process.env.MEILISEARCH_STAGING_API_KEY || '';
    
    console.log(`🔌 Connecting to Staging Meilisearch at: ${host}`);
  ...
  async initialize() {
    try {
      ...
      console.log('Connected to existing staging index:', config.indexName);
    } catch (error) {
      if (error.cause?.code === 'index_not_found') {
        // Index doesn't exist, create it
        console.log('Creating new staging index:', config.indexName);
        await this.client.createIndex(config.indexName, { primaryKey: 'id' });
        this.index = await this.client.getIndex(config.indexName);
        console.log('Created new staging index:', config.indexName);
      } else {
  ...
  async configureIndex() {
    ...
    console.log('Staging index configuration updated');
  }

  async scrapeFromSitemap() {
    console.log('Fetching staging sitemap...');
    ...
    for (const sitemapUrl of config.sitemapUrls) {
      try {
        ...
        console.error('Error fetching staging sitemap:', error.message);
      }
    }

    console.log(`Found ${this.visitedUrls.size} URLs to scrape from staging`);
  }
  ...
  async scrapePage(url) {
    try {
      console.log('Scraping staging page:', url);
      const response = await axios.get(url, {
        timeout: 10000,
        headers: {
          'User-Agent': 'Meilisearch Staging Documentation Scraper'
        }
      });
      ...
      textElements.each((index, element) => {
        ...
        if (text && text.length > 10) { // Filter out very short content
          ...
          const id = `staging_doc_${urlHash}_${index}`;
      ...
      console.error(`Error scraping staging page ${url}:`, error.message);
      return [];
    }
  }

  async scrapeAllPages() {
    console.log('Starting to scrape staging pages...');
    ...
    console.log(`Scraped ${this.documents.length} documents from staging`);
  }
  ...
  async indexDocuments() {
    ...
    console.log(`Indexing ${this.documents.length} staging documents...`);
    ...
    for (let i = 0; i < this.documents.length; i += batchSize) {
      ...
      console.log(`Indexed staging batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(this.documents.length/batchSize)}`);
    }

    console.log('Staging indexing completed!');
  }
...
if (require.main === module) {
  const indexer = new StagingMeilisearchIndexer();
  indexer.run().catch(console.error);
}

module.exports = StagingMeilisearchIndexer;
